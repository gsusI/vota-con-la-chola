#!/usr/bin/env python3
"""Append-only heartbeat lane for citizen trust-action nudge digest."""
...
DEFAULT_DIGEST_JSON = Path("docs/etl/sprints/AI-OPS-86/evidence/citizen_trust_action_nudges_latest.json")
DEFAULT_HEARTBEAT_JSONL = Path("docs/etl/runs/citizen_trust_action_nudges_heartbeat.jsonl")
...
def parse_args(argv: list[str] | None = None) -> argparse.Namespace:
    p = argparse.ArgumentParser(description="Append heartbeat JSONL for citizen trust-action nudge digest")
    p.add_argument(
        "--digest-json",
        default=str(DEFAULT_DIGEST_JSON),
        help=f"Input trust-action nudge digest JSON (default: {DEFAULT_DIGEST_JSON})",
    )
    p.add_argument(
        "--heartbeat-jsonl",
...
def build_heartbeat(
    ...
    *,
    ...
    nudge_shown_events_total = _to_int(metrics.get("nudge_shown_events_total"), 0)
    nudge_clicked_events_total = _to_int(metrics.get("nudge_clicked_events_total"), 0)
    nudge_shown_sessions_total = _to_int(metrics.get("nudge_shown_sessions_total"), 0)
    nudge_clicked_sessions_total = _to_int(metrics.get("nudge_clicked_sessions_total"), 0)
    nudge_clickthrough_session_rate = _safe_float(metrics.get("nudge_clickthrough_session_rate"))
    nudge_clickthrough_event_rate = _safe_float(metrics.get("nudge_clickthrough_event_rate"))

    min_nudge_shown_events = _to_int(thresholds.get("min_nudge_shown_events"), 0)
    min_nudge_shown_sessions = _to_int(thresholds.get("min_nudge_shown_sessions"), 0)
    min_nudge_clickthrough_rate = _safe_float(thresholds.get("min_nudge_clickthrough_rate"))

    nudge_clickthrough_meets_minimum_raw = checks.get("nudge_clickthrough_meets_minimum")
    nudge_clickthrough_meets_minimum = (
        bool(nudge_clickthrough_meets_minimum_raw) if isinstance(nudge_clickthrough_meets_minimum_raw, bool) else None
    )
    ...
    if status == "failed":
        ...
        if not failure_reasons:
            strict_fail_reasons.append("trust_action_nudges_failed_without_reason")
    ...
    heartbeat_id = "|".join(
        [
            ...
            status,
            digest_generated_at,
            str(nudge_shown_events_total),
            str(nudge_clicked_events_total),
            str(nudge_shown_sessions_total),
            str(nudge_clicked_sessions_total),
            _fmt_float(nudge_clickthrough_session_rate),
            _fmt_float(nudge_clickthrough_event_rate),
            str(min_nudge_shown_events),
            str(min_nudge_shown_sessions),
            _fmt_float(min_nudge_clickthrough_rate),
    ...
    return {
        ...
        "status": status,
        "nudge_shown_events_total": nudge_shown_events_total,
        "nudge_clicked_events_total": nudge_clicked_events_total,
        "nudge_shown_sessions_total": nudge_shown_sessions_total,
        "nudge_clicked_sessions_total": nudge_clicked_sessions_total,
        "nudge_clickthrough_session_rate": round(float(nudge_clickthrough_session_rate), 6)
        if nudge_clickthrough_session_rate is not None
        else None,
        "nudge_clickthrough_event_rate": round(float(nudge_clickthrough_event_rate), 6)
        if nudge_clickthrough_event_rate is not None
        else None,
        "min_nudge_shown_events": min_nudge_shown_events,
        "min_nudge_shown_sessions": min_nudge_shown_sessions,
        "min_nudge_clickthrough_rate": round(float(min_nudge_clickthrough_rate), 6)
        if min_nudge_clickthrough_rate is not None
        else None,
        "nudge_clickthrough_meets_minimum": nudge_clickthrough_meets_minimum,
...
def validate_heartbeat(heartbeat: dict[str, Any]) -> list[str]:
    ...
    for key in (
        ...
        "min_nudge_shown_events",
        "min_nudge_shown_sessions",
    ):
        value = _to_int(heartbeat.get(key), -1)
        if value < 0:
            reasons.append(f"invalid_{key}")

    nudge_shown_events_total = _to_int(heartbeat.get("nudge_shown_events_total"), -1)
    nudge_clicked_events_total = _to_int(heartbeat.get("nudge_clicked_events_total"), -1)
    nudge_shown_sessions_total = _to_int(heartbeat.get("nudge_shown_sessions_total"), -1)
    nudge_clicked_sessions_total = _to_int(heartbeat.get("nudge_clicked_sessions_total"), -1)

    if nudge_clicked_events_total > nudge_shown_events_total >= 0:
        reasons.append("nudge_clicked_events_exceeds_shown_events")
    if nudge_clicked_sessions_total > nudge_shown_sessions_total >= 0:
        reasons.append("nudge_clicked_sessions_exceeds_shown_sessions")

    for key in (
        "nudge_clickthrough_session_rate",
        "nudge_clickthrough_event_rate",
        "min_nudge_clickthrough_rate",
    ):
