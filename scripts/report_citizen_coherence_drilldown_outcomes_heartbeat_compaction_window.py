#!/usr/bin/env python3
"""Parity report for raw vs compacted coherence drilldown outcomes heartbeat (last-N window)."""
...
def parse_args(argv: list[str] | None = None) -> argparse.Namespace:
    p = argparse.ArgumentParser(description="Citizen coherence drilldown outcomes heartbeat compaction-window report")
    p.add_argument(
        "--heartbeat-jsonl",
...
def _to_ref(row: dict[str, Any]) -> dict[str, Any]:
    ...
    return {
        ...
        "replay_success_rate_violation": _is_threshold_violation(entry, key_ok="replay_success_rate_meets_minimum"),
        "contract_click_rate_violation": _is_threshold_violation(
            entry,
            key_ok="contract_complete_click_rate_meets_minimum",
        ),
        "replay_failure_rate_violation": _is_threshold_violation(entry, key_ok="replay_failure_rate_within_threshold"),
...
def _has_incident(ref: dict[str, Any]) -> bool:
    ...
    if not bool(ref.get("contract_complete")):
        return True
    if bool(ref.get("replay_success_rate_violation")):
        return True
    if bool(ref.get("contract_click_rate_violation")):
        return True
    if bool(ref.get("replay_failure_rate_violation")):
        return True
    return False
...
def _sample_ref(ref: dict[str, Any], *, present: bool) -> dict[str, Any]:
    return {
        ...
        "replay_success_rate_violation": bool(ref.get("replay_success_rate_violation")),
        "contract_click_rate_violation": bool(ref.get("contract_click_rate_violation")),
        "replay_failure_rate_violation": bool(ref.get("replay_failure_rate_violation")),
...
def build_compaction_window_report(
    ...
    *,
    ...
    report: dict[str, Any] = {
        ...
        "raw_window_replay_success_rate_violations": 0,
        "raw_window_contract_click_rate_violations": 0,
        "raw_window_replay_failure_rate_violations": 0,
        ...
        "replay_success_rate_violations_present_in_compacted": 0,
        "replay_success_rate_violations_missing_in_compacted": 0,
        "contract_click_rate_violations_present_in_compacted": 0,
        "contract_click_rate_violations_missing_in_compacted": 0,
        "replay_failure_rate_violations_present_in_compacted": 0,
        "replay_failure_rate_violations_missing_in_compacted": 0,
        ...
        "checks": {
            ...
            "replay_success_rate_violations_parity_ok": False,
            "contract_click_rate_violations_parity_ok": False,
            "replay_failure_rate_violations_parity_ok": False,
    ...
    for ref in raw_window_refs:
        ...
        is_replay_success_violation = bool(ref.get("replay_success_rate_violation"))
        is_contract_click_violation = bool(ref.get("contract_click_rate_violation"))
        is_replay_failure_violation = bool(ref.get("replay_failure_rate_violation"))
        ...
        if is_replay_success_violation:
            report["raw_window_replay_success_rate_violations"] += 1
        if is_contract_click_violation:
            report["raw_window_contract_click_rate_violations"] += 1
        if is_replay_failure_violation:
            report["raw_window_replay_failure_rate_violations"] += 1
        ...
        if present:
            ...
            if is_replay_success_violation:
                report["replay_success_rate_violations_present_in_compacted"] += 1
            if is_contract_click_violation:
                report["contract_click_rate_violations_present_in_compacted"] += 1
            if is_replay_failure_violation:
                report["replay_failure_rate_violations_present_in_compacted"] += 1
        else:
            ...
            if is_replay_success_violation:
                report["replay_success_rate_violations_missing_in_compacted"] += 1
            if is_contract_click_violation:
                report["contract_click_rate_violations_missing_in_compacted"] += 1
            if is_replay_failure_violation:
                report["replay_failure_rate_violations_missing_in_compacted"] += 1
    ...
    checks["replay_success_rate_violations_parity_ok"] = (
        report["replay_success_rate_violations_present_in_compacted"]
        >= report["raw_window_replay_success_rate_violations"]
    )
    checks["contract_click_rate_violations_parity_ok"] = (
        report["contract_click_rate_violations_present_in_compacted"]
        >= report["raw_window_contract_click_rate_violations"]
    )
    checks["replay_failure_rate_violations_parity_ok"] = (
        report["replay_failure_rate_violations_present_in_compacted"]
        >= report["raw_window_replay_failure_rate_violations"]
    )
    ...
    if not checks["replay_success_rate_violations_parity_ok"]:
        reasons.append("replay_success_rate_violations_underreported_in_compacted")
    if not checks["contract_click_rate_violations_parity_ok"]:
        reasons.append("contract_click_rate_violations_underreported_in_compacted")
    if not checks["replay_failure_rate_violations_parity_ok"]:
        reasons.append("replay_failure_rate_violations_underreported_in_compacted")
