#!/usr/bin/env python3
"""Windowed trend report for citizen explainability outcomes heartbeat JSONL."""
...
def parse_args(argv: list[str] | None = None) -> argparse.Namespace:
    p = argparse.ArgumentParser(description="Citizen explainability outcomes heartbeat window report")
    p.add_argument(
        "--heartbeat-jsonl",
    ...
    p.add_argument(
        ...
        type=int,
        default=DEFAULT_MAX_CONTRACT_INCOMPLETE,
        help=f"Max allowed contract-incomplete rows in window (default: {DEFAULT_MAX_CONTRACT_INCOMPLETE})",
    )
    p.add_argument(
        ...
        help=(
            "Max allowed contract-incomplete rate pct in window "
            f"(default: {DEFAULT_MAX_CONTRACT_INCOMPLETE_RATE_PCT})"
        ),
    )
    p.add_argument(
        "--strict",
...
def _latest_summary(row: dict[str, Any] | None) -> dict[str, Any]:
    if not row:
        return {
            ...
            "malformed_line": True,
            "digest_generated_at": "",
            "adoption_sessions_total": None,
            "complete_adoption_sessions_total": None,
            "adoption_completeness_rate": None,
    ...
    return {
        ...
        "adoption_sessions_total": _to_int(entry.get("adoption_sessions_total"), 0)
        if "adoption_sessions_total" in entry
        else None,
        "complete_adoption_sessions_total": _to_int(entry.get("complete_adoption_sessions_total"), 0)
        if "complete_adoption_sessions_total" in entry
        else None,
        "adoption_completeness_rate": _safe_float(entry.get("adoption_completeness_rate")),
...
def build_window_report(
    ...
    *,
    ...
    if report["entries_in_window"] > 0:
        ...
        report["contract_incomplete_rate_pct"] = _round4((float(report["contract_incomplete_in_window"]) / denom) * 100.0)
    ...
    checks["max_contract_incomplete_rate_ok"] = (
        float(report["contract_incomplete_rate_pct"]) <= max_contract_incomplete_rate
    )
    ...
    if report["entries_in_window"] == 0:
        ...
        or not checks["max_degraded_ok"]
        or not checks["max_degraded_rate_ok"]
        or not checks["max_contract_incomplete_ok"]
        or not checks["max_contract_incomplete_rate_ok"]
        or not checks["latest_not_failed_ok"]
        or not checks["latest_contract_complete_ok"]
    ):
        report["status"] = "failed"
    elif report["degraded_in_window"] > 0 or report["contract_incomplete_in_window"] > 0:
        report["status"] = "degraded"
    else:
        report["status"] = "ok"
