#!/usr/bin/env python3
"""Compaction reporter for sanction procedural packet-fix queue heartbeat JSONL."""
...
DEFAULT_COMPACTED_JSONL = Path(
    "docs/etl/runs/sanction_procedural_official_review_packet_fix_queue_heartbeat.compacted.jsonl"
)
...
def parse_args(argv: list[str] | None = None) -> argparse.Namespace:
    p = argparse.ArgumentParser(description="Packet-fix queue heartbeat compaction report")
    p.add_argument(
        "--heartbeat-jsonl",
    ...
    p.add_argument("--keep-mid-span", type=int, default=DEFAULT_KEEP_MID_SPAN)
    p.add_argument("--keep-mid-every", type=int, default=DEFAULT_KEEP_MID_EVERY)
    p.add_argument("--keep-old-every", type=int, default=DEFAULT_KEEP_OLD_EVERY)
    p.add_argument("--min-raw-for-dropped-check", type=int, default=DEFAULT_MIN_RAW_FOR_DROPPED_CHECK)
...
def build_compaction_report(
    ...
    *,
    ...
    # Rows with incidents are mandatory-preserved by design. Anchors are also mandatory-preserved even
    # when non-incident; do not enforce "must drop" when only mandatory rows exist above threshold.
    non_incident_anchor_indices: set[int] = set()
    if entries_total > 0 and not _has_incident(rows[0]):
        non_incident_anchor_indices.add(0)
    if entries_total > 1 and not _has_incident(rows[-1]):
        non_incident_anchor_indices.add(entries_total - 1)
    non_incident_anchor_total = len(non_incident_anchor_indices)
    drop_candidates_total = max(entries_total - incidents_total - non_incident_anchor_total, 0)
    ...
    report: dict[str, Any] = {
        ...
        "drop_candidates_total": drop_candidates_total,
        "non_incident_anchor_total": non_incident_anchor_total,
        "tiers": tiers,
    ...
    if entries_total >= min_raw_n and drop_candidates_total > 0:
