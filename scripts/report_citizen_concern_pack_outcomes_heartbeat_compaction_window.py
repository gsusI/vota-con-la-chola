#!/usr/bin/env python3
"""Parity report for raw vs compacted concern-pack outcomes heartbeat (last-N window)."""
...
def parse_args(argv: list[str] | None = None) -> argparse.Namespace:
    p = argparse.ArgumentParser(description="Citizen concern-pack outcomes heartbeat compaction-window report")
    p.add_argument(
        "--heartbeat-jsonl",
...
def _to_ref(row: dict[str, Any]) -> dict[str, Any]:
    ...
    return {
        ...
        "weak_pack_followthrough_violation": _is_threshold_violation(entry, key_ok="weak_pack_followthrough_rate_meets_minimum"),
        "unknown_pack_select_share_violation": _is_threshold_violation(
            entry,
            key_ok="unknown_pack_select_share_within_threshold",
        ),
...
def _has_incident(ref: dict[str, Any]) -> bool:
    ...
    if not bool(ref.get("contract_complete")):
        return True
    if bool(ref.get("weak_pack_followthrough_violation")):
        return True
    if bool(ref.get("unknown_pack_select_share_violation")):
        return True
    return False
...
def _sample_ref(ref: dict[str, Any], *, present: bool) -> dict[str, Any]:
    return {
        ...
        "weak_pack_followthrough_violation": bool(ref.get("weak_pack_followthrough_violation")),
        "unknown_pack_select_share_violation": bool(ref.get("unknown_pack_select_share_violation")),
    ...
    *,
    ...
    report: dict[str, Any] = {
        ...
        "checks": {
            ...
            "weak_pack_followthrough_violations_parity_ok": False,
            "unknown_pack_select_share_violations_parity_ok": False,
    ...
    for ref in raw_window_refs:
        ...
        is_followthrough_violation = bool(ref.get("weak_pack_followthrough_violation"))
        is_unknown_share_violation = bool(ref.get("unknown_pack_select_share_violation"))
        ...
        if is_followthrough_violation:
            report["raw_window_weak_pack_followthrough_violations"] += 1
        if is_unknown_share_violation:
            report["raw_window_unknown_pack_select_share_violations"] += 1
        ...
        if present:
            ...
            if is_followthrough_violation:
                report["weak_pack_followthrough_violations_present_in_compacted"] += 1
            if is_unknown_share_violation:
                report["unknown_pack_select_share_violations_present_in_compacted"] += 1
        else:
            ...
            if is_followthrough_violation:
                report["weak_pack_followthrough_violations_missing_in_compacted"] += 1
            if is_unknown_share_violation:
                report["unknown_pack_select_share_violations_missing_in_compacted"] += 1
    ...
    checks["weak_pack_followthrough_violations_parity_ok"] = (
        report["weak_pack_followthrough_violations_present_in_compacted"]
        >= report["raw_window_weak_pack_followthrough_violations"]
    )
    checks["unknown_pack_select_share_violations_parity_ok"] = (
        report["unknown_pack_select_share_violations_present_in_compacted"]
        >= report["raw_window_unknown_pack_select_share_violations"]
    )
    ...
    if not checks["weak_pack_followthrough_violations_parity_ok"]:
        reasons.append("weak_pack_followthrough_violations_underreported_in_compacted")
    if not checks["unknown_pack_select_share_violations_parity_ok"]:
        reasons.append("unknown_pack_select_share_violations_underreported_in_compacted")
