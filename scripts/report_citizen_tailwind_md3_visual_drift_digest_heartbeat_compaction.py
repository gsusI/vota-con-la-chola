#!/usr/bin/env python3
"""Compaction reporter for Tailwind+MD3 visual-drift heartbeat JSONL."""
...
def _field_is_false_or_missing(entry: dict[str, Any], key: str) -> bool:
    raw = entry.get(key)
...
def parse_args(argv: list[str] | None = None) -> argparse.Namespace:
    p = argparse.ArgumentParser(description="Tailwind+MD3 visual-drift heartbeat compaction report")
    p.add_argument(
        "--heartbeat-jsonl",
...
def _has_incident(row: dict[str, Any]) -> bool:
    ...
    if _field_is_false_or_missing(entry, "tailwind_contract_exists"):
        return True
    if _field_is_false_or_missing(entry, "tailwind_contract_status_ok"):
        return True
    if _field_is_false_or_missing(entry, "tailwind_contract_checks_ok"):
        return True

    if _field_is_false_or_missing(entry, "source_published_parity_ok"):
        return True
    if _field_is_false_or_missing(entry, "marker_parity_ok"):
        return True
    if _field_is_false_or_missing(entry, "tokens_parity_ok"):
        return True
    if _field_is_false_or_missing(entry, "tokens_data_parity_ok"):
        return True
    if _field_is_false_or_missing(entry, "css_parity_ok"):
        return True
    if _field_is_false_or_missing(entry, "ui_html_parity_ok"):
        return True

    if _to_int(entry.get("parity_fail_count"), 0) > 0:
        return True
    if len(_safe_list_str(entry.get("parity_fail_reasons"))) > 0:
        return True
...
def build_compaction_report(
    ...
    *,
    ...
    contract_exists_missing_total = 0
    contract_exists_missing_selected = 0
    contract_status_fail_total = 0
    contract_status_fail_selected = 0
    contract_checks_fail_total = 0
    contract_checks_fail_selected = 0

    source_published_parity_mismatch_total = 0
    source_published_parity_mismatch_selected = 0
    marker_parity_mismatch_total = 0
    marker_parity_mismatch_selected = 0
    tokens_parity_mismatch_total = 0
    tokens_parity_mismatch_selected = 0
    tokens_data_parity_mismatch_total = 0
    tokens_data_parity_mismatch_selected = 0
    css_parity_mismatch_total = 0
    css_parity_mismatch_selected = 0
    ui_html_parity_mismatch_total = 0
    ui_html_parity_mismatch_selected = 0

    parity_fail_rows_total = 0
    parity_fail_rows_selected = 0
    ...
    for i, row in enumerate(rows):
        ...
        has_contract_exists_missing = _field_is_false_or_missing(entry, "tailwind_contract_exists")
        has_contract_status_fail = _field_is_false_or_missing(entry, "tailwind_contract_status_ok")
        has_contract_checks_fail = _field_is_false_or_missing(entry, "tailwind_contract_checks_ok")

        has_source_published_parity_mismatch = _field_is_false_or_missing(entry, "source_published_parity_ok")
        has_marker_parity_mismatch = _field_is_false_or_missing(entry, "marker_parity_ok")
        has_tokens_parity_mismatch = _field_is_false_or_missing(entry, "tokens_parity_ok")
        has_tokens_data_parity_mismatch = _field_is_false_or_missing(entry, "tokens_data_parity_ok")
        has_css_parity_mismatch = _field_is_false_or_missing(entry, "css_parity_ok")
        has_ui_html_parity_mismatch = _field_is_false_or_missing(entry, "ui_html_parity_ok")

        has_parity_fail_row = _to_int(entry.get("parity_fail_count"), 0) > 0 or len(_safe_list_str(entry.get("parity_fail_reasons"))) > 0
        ...
        if has_contract_exists_missing:
            contract_exists_missing_total += 1
            if i in selected_idx:
                contract_exists_missing_selected += 1

        if has_contract_status_fail:
            contract_status_fail_total += 1
            if i in selected_idx:
                contract_status_fail_selected += 1

        if has_contract_checks_fail:
            contract_checks_fail_total += 1
            if i in selected_idx:
                contract_checks_fail_selected += 1

        if has_source_published_parity_mismatch:
            source_published_parity_mismatch_total += 1
            if i in selected_idx:
                source_published_parity_mismatch_selected += 1

        if has_marker_parity_mismatch:
            marker_parity_mismatch_total += 1
            if i in selected_idx:
                marker_parity_mismatch_selected += 1

        if has_tokens_parity_mismatch:
            tokens_parity_mismatch_total += 1
            if i in selected_idx:
                tokens_parity_mismatch_selected += 1

        if has_tokens_data_parity_mismatch:
            tokens_data_parity_mismatch_total += 1
            if i in selected_idx:
                tokens_data_parity_mismatch_selected += 1

        if has_css_parity_mismatch:
            css_parity_mismatch_total += 1
            if i in selected_idx:
                css_parity_mismatch_selected += 1

        if has_ui_html_parity_mismatch:
            ui_html_parity_mismatch_total += 1
            if i in selected_idx:
                ui_html_parity_mismatch_selected += 1

        if has_parity_fail_row:
            parity_fail_rows_total += 1
            if i in selected_idx:
                parity_fail_rows_selected += 1
    ...
    report: dict[str, Any] = {
        ...
        "contract_exists_missing_total": contract_exists_missing_total,
        "contract_exists_missing_selected": contract_exists_missing_selected,
        "contract_exists_missing_dropped": contract_exists_missing_total - contract_exists_missing_selected,
        "contract_status_fail_total": contract_status_fail_total,
        "contract_status_fail_selected": contract_status_fail_selected,
        "contract_status_fail_dropped": contract_status_fail_total - contract_status_fail_selected,
        "contract_checks_fail_total": contract_checks_fail_total,
        "contract_checks_fail_selected": contract_checks_fail_selected,
        "contract_checks_fail_dropped": contract_checks_fail_total - contract_checks_fail_selected,
        "source_published_parity_mismatch_total": source_published_parity_mismatch_total,
        "source_published_parity_mismatch_selected": source_published_parity_mismatch_selected,
        "source_published_parity_mismatch_dropped": source_published_parity_mismatch_total - source_published_parity_mismatch_selected,
        "marker_parity_mismatch_total": marker_parity_mismatch_total,
        "marker_parity_mismatch_selected": marker_parity_mismatch_selected,
        "marker_parity_mismatch_dropped": marker_parity_mismatch_total - marker_parity_mismatch_selected,
        "tokens_parity_mismatch_total": tokens_parity_mismatch_total,
        "tokens_parity_mismatch_selected": tokens_parity_mismatch_selected,
        "tokens_parity_mismatch_dropped": tokens_parity_mismatch_total - tokens_parity_mismatch_selected,
        "tokens_data_parity_mismatch_total": tokens_data_parity_mismatch_total,
        "tokens_data_parity_mismatch_selected": tokens_data_parity_mismatch_selected,
        "tokens_data_parity_mismatch_dropped": tokens_data_parity_mismatch_total - tokens_data_parity_mismatch_selected,
        "css_parity_mismatch_total": css_parity_mismatch_total,
        "css_parity_mismatch_selected": css_parity_mismatch_selected,
        "css_parity_mismatch_dropped": css_parity_mismatch_total - css_parity_mismatch_selected,
        "ui_html_parity_mismatch_total": ui_html_parity_mismatch_total,
        "ui_html_parity_mismatch_selected": ui_html_parity_mismatch_selected,
        "ui_html_parity_mismatch_dropped": ui_html_parity_mismatch_total - ui_html_parity_mismatch_selected,
        "parity_fail_rows_total": parity_fail_rows_total,
        "parity_fail_rows_selected": parity_fail_rows_selected,
        "parity_fail_rows_dropped": parity_fail_rows_total - parity_fail_rows_selected,
        "anchors": {
        ...
        "checks": {
            ...
            "contract_exists_preserved_ok": contract_exists_missing_total == contract_exists_missing_selected,
            "contract_status_preserved_ok": contract_status_fail_total == contract_status_fail_selected,
            "contract_checks_preserved_ok": contract_checks_fail_total == contract_checks_fail_selected,
            "source_published_parity_preserved_ok": source_published_parity_mismatch_total == source_published_parity_mismatch_selected,
            "marker_parity_preserved_ok": marker_parity_mismatch_total == marker_parity_mismatch_selected,
            "tokens_parity_preserved_ok": tokens_parity_mismatch_total == tokens_parity_mismatch_selected,
            "tokens_data_parity_preserved_ok": tokens_data_parity_mismatch_total == tokens_data_parity_mismatch_selected,
            "css_parity_preserved_ok": css_parity_mismatch_total == css_parity_mismatch_selected,
            "ui_html_parity_preserved_ok": ui_html_parity_mismatch_total == ui_html_parity_mismatch_selected,
            "parity_fail_rows_preserved_ok": parity_fail_rows_total == parity_fail_rows_selected,
    ...
    if not report["checks"]["contract_exists_preserved_ok"]:
        reasons.append("contract_exists_missing_rows_dropped")
    if not report["checks"]["contract_status_preserved_ok"]:
        reasons.append("contract_status_fail_rows_dropped")
    if not report["checks"]["contract_checks_preserved_ok"]:
        reasons.append("contract_checks_fail_rows_dropped")
    if not report["checks"]["source_published_parity_preserved_ok"]:
        reasons.append("source_published_parity_mismatch_rows_dropped")
    if not report["checks"]["marker_parity_preserved_ok"]:
        reasons.append("marker_parity_mismatch_rows_dropped")
    if not report["checks"]["tokens_parity_preserved_ok"]:
        reasons.append("tokens_parity_mismatch_rows_dropped")
    if not report["checks"]["tokens_data_parity_preserved_ok"]:
        reasons.append("tokens_data_parity_mismatch_rows_dropped")
    if not report["checks"]["css_parity_preserved_ok"]:
        reasons.append("css_parity_mismatch_rows_dropped")
    if not report["checks"]["ui_html_parity_preserved_ok"]:
        reasons.append("ui_html_parity_mismatch_rows_dropped")
    if not report["checks"]["parity_fail_rows_preserved_ok"]:
        reasons.append("parity_fail_rows_dropped")
