#!/usr/bin/env python3
"""Compaction reporter for citizen coherence drilldown outcomes heartbeat JSONL."""
...
DEFAULT_COMPACTED_JSONL = Path("docs/etl/runs/citizen_coherence_drilldown_outcomes_heartbeat.compacted.jsonl")
...
def parse_args(argv: list[str] | None = None) -> argparse.Namespace:
    p = argparse.ArgumentParser(description="Citizen coherence drilldown outcomes heartbeat compaction report")
    p.add_argument(
        "--heartbeat-jsonl",
...
def _has_incident(row: dict[str, Any]) -> bool:
    ...
    if not bool(entry.get("contract_complete")):
        return True
...
def build_compaction_report(
    ...
    *,
    ...
    replay_success_rate_violations_total = 0
    replay_success_rate_violations_selected = 0
    contract_click_rate_violations_total = 0
    contract_click_rate_violations_selected = 0
    replay_failure_rate_violations_total = 0
    replay_failure_rate_violations_selected = 0
    ...
    for i, row in enumerate(rows):
        ...
        has_contract_incomplete = not bool(entry.get("contract_complete"))
        has_replay_success_violation = _is_threshold_violation(entry, key_ok="replay_success_rate_meets_minimum")
        has_contract_click_violation = _is_threshold_violation(entry, key_ok="contract_complete_click_rate_meets_minimum")
        has_replay_failure_violation = _is_threshold_violation(entry, key_ok="replay_failure_rate_within_threshold")
        ...
        if has_contract_incomplete:
            contract_incomplete_total += 1
            if i in selected_idx:
                contract_incomplete_selected += 1

        if has_replay_success_violation:
            replay_success_rate_violations_total += 1
            if i in selected_idx:
                replay_success_rate_violations_selected += 1

        if has_contract_click_violation:
            contract_click_rate_violations_total += 1
            if i in selected_idx:
                contract_click_rate_violations_selected += 1

        if has_replay_failure_violation:
            replay_failure_rate_violations_total += 1
            if i in selected_idx:
                replay_failure_rate_violations_selected += 1
    ...
    report: dict[str, Any] = {
        ...
        "contract_incomplete_total": contract_incomplete_total,
        "contract_incomplete_selected": contract_incomplete_selected,
        "contract_incomplete_dropped": contract_incomplete_total - contract_incomplete_selected,
        "replay_success_rate_violations_total": replay_success_rate_violations_total,
        "replay_success_rate_violations_selected": replay_success_rate_violations_selected,
        "replay_success_rate_violations_dropped": replay_success_rate_violations_total - replay_success_rate_violations_selected,
        "contract_click_rate_violations_total": contract_click_rate_violations_total,
        "contract_click_rate_violations_selected": contract_click_rate_violations_selected,
        "contract_click_rate_violations_dropped": contract_click_rate_violations_total - contract_click_rate_violations_selected,
        "replay_failure_rate_violations_total": replay_failure_rate_violations_total,
        "replay_failure_rate_violations_selected": replay_failure_rate_violations_selected,
        "replay_failure_rate_violations_dropped": replay_failure_rate_violations_total - replay_failure_rate_violations_selected,
        "anchors": {
        ...
        "checks": {
            ...
            "contract_incomplete_preserved_ok": contract_incomplete_total == contract_incomplete_selected,
            "replay_success_rate_violations_preserved_ok": replay_success_rate_violations_total
            == replay_success_rate_violations_selected,
            "contract_click_rate_violations_preserved_ok": contract_click_rate_violations_total
            == contract_click_rate_violations_selected,
            "replay_failure_rate_violations_preserved_ok": replay_failure_rate_violations_total
            == replay_failure_rate_violations_selected,
    ...
    if not report["checks"]["contract_incomplete_preserved_ok"]:
        reasons.append("contract_incomplete_rows_dropped")
    if not report["checks"]["replay_success_rate_violations_preserved_ok"]:
        reasons.append("replay_success_rate_violation_rows_dropped")
    if not report["checks"]["contract_click_rate_violations_preserved_ok"]:
        reasons.append("contract_click_rate_violation_rows_dropped")
    if not report["checks"]["replay_failure_rate_violations_preserved_ok"]:
        reasons.append("replay_failure_rate_violation_rows_dropped")
