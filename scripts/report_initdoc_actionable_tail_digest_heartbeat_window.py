#!/usr/bin/env python3
"""Windowed trend report for initdoc actionable-tail digest heartbeat JSONL."""
...
def parse_args(argv: list[str] | None = None) -> argparse.Namespace:
    p = argparse.ArgumentParser(description="Initdoc actionable-tail digest heartbeat window report")
    p.add_argument(
        "--heartbeat-jsonl",
...
def _latest_summary(row: dict[str, Any] | None) -> dict[str, Any]:
    if not row:
        return {"run_at": "", "heartbeat_id": "", "status": "failed", "line_no": 0, "malformed_line": True}
