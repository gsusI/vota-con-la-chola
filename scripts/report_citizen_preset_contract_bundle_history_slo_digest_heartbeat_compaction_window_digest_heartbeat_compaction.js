...
function usage() {
  return [
    "Usage:",
    "  node scripts/report_citizen_preset_contract_bundle_history_slo_digest_heartbeat_compaction_window_digest_heartbeat_compaction.js [options]",
    "",
    "Options:",
...
function summarizeRows(rows, selection, args) {
  ...
  for (let i = 0; i < n; i += 1) {
    ...
    if (status === "degraded") {
      degradedTotal += 1;
      if (selected) degradedSelected += 1;
    }
...
function main() {
  ...
  try {
    ...
    report.degraded_total = summary.degradedTotal;
    report.degraded_selected = summary.degradedSelected;
