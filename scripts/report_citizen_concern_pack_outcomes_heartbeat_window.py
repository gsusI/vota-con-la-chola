#!/usr/bin/env python3
"""Windowed trend report for citizen concern-pack outcomes heartbeat JSONL."""
...
DEFAULT_MAX_WEAK_PACK_FOLLOWTHROUGH_VIOLATIONS = 0
DEFAULT_MAX_WEAK_PACK_FOLLOWTHROUGH_VIOLATION_RATE_PCT = 0.0
DEFAULT_MAX_UNKNOWN_PACK_SELECT_SHARE_VIOLATIONS = 0
DEFAULT_MAX_UNKNOWN_PACK_SELECT_SHARE_VIOLATION_RATE_PCT = 0.0
...
def parse_args(argv: list[str] | None = None) -> argparse.Namespace:
    p = argparse.ArgumentParser(description="Citizen concern-pack outcomes heartbeat window report")
    p.add_argument(
        "--heartbeat-jsonl",
    ...
    p.add_argument(
        ...
        type=int,
        default=DEFAULT_MAX_WEAK_PACK_FOLLOWTHROUGH_VIOLATIONS,
    )
    p.add_argument(
        "--max-weak-pack-followthrough-violation-rate-pct",
        type=float,
        default=DEFAULT_MAX_WEAK_PACK_FOLLOWTHROUGH_VIOLATION_RATE_PCT,
    )
    p.add_argument(
        "--max-unknown-pack-select-share-violations",
        type=int,
        default=DEFAULT_MAX_UNKNOWN_PACK_SELECT_SHARE_VIOLATIONS,
    )
    p.add_argument(
        "--max-unknown-pack-select-share-violation-rate-pct",
        type=float,
        default=DEFAULT_MAX_UNKNOWN_PACK_SELECT_SHARE_VIOLATION_RATE_PCT,
    )
...
def _latest_summary(row: dict[str, Any] | None) -> dict[str, Any]:
    if not row:
        return {
            ...
            "malformed_line": True,
            "digest_generated_at": "",
            "weak_pack_followthrough_rate": None,
            "unknown_pack_select_share": None,
            "contract_complete": None,
            "weak_pack_followthrough_rate_meets_minimum": None,
            "unknown_pack_select_share_within_threshold": None,
        }
    ...
    return {
        ...
        "weak_pack_followthrough_rate": _safe_float(entry.get("weak_pack_followthrough_rate")),
        "unknown_pack_select_share": _safe_float(entry.get("unknown_pack_select_share")),
        "contract_complete": bool(entry.get("contract_complete")) if "contract_complete" in entry else None,
        "weak_pack_followthrough_rate_meets_minimum": entry.get("weak_pack_followthrough_rate_meets_minimum")
        if isinstance(entry.get("weak_pack_followthrough_rate_meets_minimum"), bool)
        else None,
        "unknown_pack_select_share_within_threshold": entry.get("unknown_pack_select_share_within_threshold")
        if isinstance(entry.get("unknown_pack_select_share_within_threshold"), bool)
        else None,
    }
...
def build_window_report(
    ...
    *,
    ...
    max_weak_pack_followthrough_violations: int = DEFAULT_MAX_WEAK_PACK_FOLLOWTHROUGH_VIOLATIONS,
    max_weak_pack_followthrough_violation_rate_pct: float = DEFAULT_MAX_WEAK_PACK_FOLLOWTHROUGH_VIOLATION_RATE_PCT,
    max_unknown_pack_select_share_violations: int = DEFAULT_MAX_UNKNOWN_PACK_SELECT_SHARE_VIOLATIONS,
    max_unknown_pack_select_share_violation_rate_pct: float = DEFAULT_MAX_UNKNOWN_PACK_SELECT_SHARE_VIOLATION_RATE_PCT,
    strict: bool = False,
    heartbeat_path: str = "",
) -> dict[str, Any]:
    ...
    max_followthrough_viol_n = _parse_non_negative_int(
        max_weak_pack_followthrough_violations,
        arg_name="max_weak_pack_followthrough_violations",
    )
    max_followthrough_viol_rate = _parse_non_negative_float(
        max_weak_pack_followthrough_violation_rate_pct,
        arg_name="max_weak_pack_followthrough_violation_rate_pct",
    )
    max_unknown_share_viol_n = _parse_non_negative_int(
        max_unknown_pack_select_share_violations,
        arg_name="max_unknown_pack_select_share_violations",
    )
    max_unknown_share_viol_rate = _parse_non_negative_float(
        max_unknown_pack_select_share_violation_rate_pct,
        arg_name="max_unknown_pack_select_share_violation_rate_pct",
    )
    ...
    report: dict[str, Any] = {
        ...
        "thresholds": {
            ...
            "max_weak_pack_followthrough_violations": int(max_followthrough_viol_n),
            "max_weak_pack_followthrough_violation_rate_pct": float(max_followthrough_viol_rate),
            "max_unknown_pack_select_share_violations": int(max_unknown_share_viol_n),
            "max_unknown_pack_select_share_violation_rate_pct": float(max_unknown_share_viol_rate),
        },
    ...
    for row in window_rows:
        ...
        if _is_threshold_violation(entry, key_ok="weak_pack_followthrough_rate_meets_minimum"):
            report["weak_pack_followthrough_violations_in_window"] = (
                int(report["weak_pack_followthrough_violations_in_window"]) + 1
            )
        if _is_threshold_violation(entry, key_ok="unknown_pack_select_share_within_threshold"):
            report["unknown_pack_select_share_violations_in_window"] = (
                int(report["unknown_pack_select_share_violations_in_window"]) + 1
            )
    ...
    if report["entries_in_window"] > 0:
        ...
        report["weak_pack_followthrough_violation_rate_pct"] = _rate_pct(
            int(report["weak_pack_followthrough_violations_in_window"]),
            denom,
        )
        report["unknown_pack_select_share_violation_rate_pct"] = _rate_pct(
            int(report["unknown_pack_select_share_violations_in_window"]),
            denom,
        )
    ...
    checks["max_weak_pack_followthrough_violations_ok"] = (
        report["weak_pack_followthrough_violations_in_window"] <= max_followthrough_viol_n
    )
    checks["max_weak_pack_followthrough_violation_rate_ok"] = (
        float(report["weak_pack_followthrough_violation_rate_pct"]) <= max_followthrough_viol_rate
    )
    checks["max_unknown_pack_select_share_violations_ok"] = (
        report["unknown_pack_select_share_violations_in_window"] <= max_unknown_share_viol_n
    )
    checks["max_unknown_pack_select_share_violation_rate_ok"] = (
        float(report["unknown_pack_select_share_violation_rate_pct"]) <= max_unknown_share_viol_rate
    )
    ...
    checks["latest_thresholds_ok"] = (
        report["entries_in_window"] > 0
        and not bool(_safe_obj(report["latest"]).get("malformed_line"))
        and bool(_safe_obj(report["latest"]).get("weak_pack_followthrough_rate_meets_minimum"))
        and bool(_safe_obj(report["latest"]).get("unknown_pack_select_share_within_threshold"))
    )
    ...
    if not checks["max_weak_pack_followthrough_violations_ok"]:
        reasons.append("max_weak_pack_followthrough_violations_exceeded")
    if not checks["max_weak_pack_followthrough_violation_rate_ok"]:
        reasons.append("max_weak_pack_followthrough_violation_rate_exceeded")
    if not checks["max_unknown_pack_select_share_violations_ok"]:
        reasons.append("max_unknown_pack_select_share_violations_exceeded")
    if not checks["max_unknown_pack_select_share_violation_rate_ok"]:
        reasons.append("max_unknown_pack_select_share_violation_rate_exceeded")
    ...
    if report["entries_in_window"] == 0:
        ...
        or report["weak_pack_followthrough_violations_in_window"] > 0
        or report["unknown_pack_select_share_violations_in_window"] > 0
    ):
        report["status"] = "degraded"
    else:
        report["status"] = "ok"
...
def build_window_report_from_path(
    ...
    *,
    ...
    return build_window_report(
        ...
        max_weak_pack_followthrough_violations=max_weak_pack_followthrough_violations,
        max_weak_pack_followthrough_violation_rate_pct=max_weak_pack_followthrough_violation_rate_pct,
        max_unknown_pack_select_share_violations=max_unknown_pack_select_share_violations,
        max_unknown_pack_select_share_violation_rate_pct=max_unknown_pack_select_share_violation_rate_pct,
        strict=strict,
...
def main(argv: list[str] | None = None) -> int:
    ...
    try:
        report = build_window_report_from_path(
            ...
            max_weak_pack_followthrough_violations=int(args.max_weak_pack_followthrough_violations),
            max_weak_pack_followthrough_violation_rate_pct=float(args.max_weak_pack_followthrough_violation_rate_pct),
            max_unknown_pack_select_share_violations=int(args.max_unknown_pack_select_share_violations),
            max_unknown_pack_select_share_violation_rate_pct=float(args.max_unknown_pack_select_share_violation_rate_pct),
