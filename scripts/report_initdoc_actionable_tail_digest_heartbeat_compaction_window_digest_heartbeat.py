#!/usr/bin/env python3
"""Append-only heartbeat lane for initdoc compact-window digest."""
...
def parse_args(argv: list[str] | None = None) -> argparse.Namespace:
    p = argparse.ArgumentParser(description="Append heartbeat JSONL for initdoc compaction-window digest")
    p.add_argument(
        "--digest-json",
        required=True,
        help=("Input digest JSON generated by report_initdoc_actionable_tail_digest_heartbeat_compaction_window_digest.py"),
    )
    p.add_argument(
        "--heartbeat-jsonl",
