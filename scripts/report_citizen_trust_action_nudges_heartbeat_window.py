#!/usr/bin/env python3
"""Windowed trend report for citizen trust-action nudge heartbeat JSONL."""
...
DEFAULT_MAX_NUDGE_CLICKTHROUGH_VIOLATIONS = 0
DEFAULT_MAX_NUDGE_CLICKTHROUGH_VIOLATION_RATE_PCT = 0.0
...
def parse_args(argv: list[str] | None = None) -> argparse.Namespace:
    p = argparse.ArgumentParser(description="Citizen trust-action nudge heartbeat window report")
    p.add_argument(
        "--heartbeat-jsonl",
    ...
    p.add_argument("--max-nudge-clickthrough-violations", type=int, default=DEFAULT_MAX_NUDGE_CLICKTHROUGH_VIOLATIONS)
    p.add_argument(
        "--max-nudge-clickthrough-violation-rate-pct",
        type=float,
        default=DEFAULT_MAX_NUDGE_CLICKTHROUGH_VIOLATION_RATE_PCT,
    )
...
def _latest_summary(row: dict[str, Any] | None) -> dict[str, Any]:
    if not row:
        return {
            ...
            "malformed_line": True,
            "digest_generated_at": "",
            "nudge_clickthrough_session_rate": None,
            "nudge_clickthrough_event_rate": None,
            "contract_complete": None,
            "nudge_clickthrough_meets_minimum": None,
        }
    ...
    return {
        ...
        "nudge_clickthrough_session_rate": _safe_float(entry.get("nudge_clickthrough_session_rate")),
        "nudge_clickthrough_event_rate": _safe_float(entry.get("nudge_clickthrough_event_rate")),
        "contract_complete": bool(entry.get("contract_complete")) if "contract_complete" in entry else None,
        "nudge_clickthrough_meets_minimum": entry.get("nudge_clickthrough_meets_minimum")
        if isinstance(entry.get("nudge_clickthrough_meets_minimum"), bool)
        else None,
    }
...
def build_window_report(
    ...
    *,
    ...
    max_nudge_clickthrough_violations: int = DEFAULT_MAX_NUDGE_CLICKTHROUGH_VIOLATIONS,
    max_nudge_clickthrough_violation_rate_pct: float = DEFAULT_MAX_NUDGE_CLICKTHROUGH_VIOLATION_RATE_PCT,
    strict: bool = False,
    heartbeat_path: str = "",
) -> dict[str, Any]:
    ...
    max_clickthrough_viol_n = _parse_non_negative_int(
        max_nudge_clickthrough_violations,
        arg_name="max_nudge_clickthrough_violations",
    )
    max_clickthrough_viol_rate = _parse_non_negative_float(
        max_nudge_clickthrough_violation_rate_pct,
        arg_name="max_nudge_clickthrough_violation_rate_pct",
    )
    ...
    report: dict[str, Any] = {
        ...
        "thresholds": {
            ...
            "max_nudge_clickthrough_violations": int(max_clickthrough_viol_n),
            "max_nudge_clickthrough_violation_rate_pct": float(max_clickthrough_viol_rate),
        },
    ...
    for row in window_rows:
        ...
        if _is_threshold_violation(entry, key_ok="nudge_clickthrough_meets_minimum"):
            report["nudge_clickthrough_violations_in_window"] = int(report["nudge_clickthrough_violations_in_window"]) + 1
    ...
    if report["entries_in_window"] > 0:
        ...
        report["nudge_clickthrough_violation_rate_pct"] = _rate_pct(
            int(report["nudge_clickthrough_violations_in_window"]),
            denom,
        )
    ...
    checks["max_nudge_clickthrough_violations_ok"] = report["nudge_clickthrough_violations_in_window"] <= max_clickthrough_viol_n
    checks["max_nudge_clickthrough_violation_rate_ok"] = (
        float(report["nudge_clickthrough_violation_rate_pct"]) <= max_clickthrough_viol_rate
    )
    ...
    checks["latest_thresholds_ok"] = (
        report["entries_in_window"] > 0
        and not bool(_safe_obj(report["latest"]).get("malformed_line"))
        and bool(_safe_obj(report["latest"]).get("nudge_clickthrough_meets_minimum"))
    )
    ...
    if not checks["max_nudge_clickthrough_violations_ok"]:
        reasons.append("max_nudge_clickthrough_violations_exceeded")
    if not checks["max_nudge_clickthrough_violation_rate_ok"]:
        reasons.append("max_nudge_clickthrough_violation_rate_exceeded")
    ...
    if report["entries_in_window"] == 0:
        ...
        or report["nudge_clickthrough_violations_in_window"] > 0
    ):
        report["status"] = "degraded"
    else:
        report["status"] = "ok"
...
def build_window_report_from_path(
    ...
    *,
    ...
    return build_window_report(
        ...
        max_nudge_clickthrough_violations=max_nudge_clickthrough_violations,
        max_nudge_clickthrough_violation_rate_pct=max_nudge_clickthrough_violation_rate_pct,
        strict=strict,
...
def main(argv: list[str] | None = None) -> int:
    ...
    try:
        report = build_window_report_from_path(
            ...
            max_nudge_clickthrough_violations=int(args.max_nudge_clickthrough_violations),
            max_nudge_clickthrough_violation_rate_pct=float(args.max_nudge_clickthrough_violation_rate_pct),
