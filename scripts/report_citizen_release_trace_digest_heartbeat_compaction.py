#!/usr/bin/env python3
"""Compaction reporter for citizen release-trace digest heartbeat JSONL."""
...
def parse_args(argv: list[str] | None = None) -> argparse.Namespace:
    p = argparse.ArgumentParser(description="Citizen release-trace digest heartbeat compaction report")
    p.add_argument(
        "--heartbeat-jsonl",
...
def _has_incident(row: dict[str, Any]) -> bool:
    ...
    if len(_safe_list_str(entry.get("strict_fail_reasons"))) > 0:
        return True
    if _stale_detected(entry) is True:
        return True
    return False
...
def build_compaction_report(
    ...
    *,
    ...
    for i, row in enumerate(rows):
        ...
        has_stale_row = _stale_detected(entry) is True
    ...
    report: dict[str, Any] = {
        ...
        "stale_rows_selected": stale_rows_selected,
        "stale_rows_dropped": stale_rows_total - stale_rows_selected,
        "anchors": {
        ...
        "checks": {
            ...
            "stale_rows_preserved_ok": stale_rows_total == stale_rows_selected,
    ...
    if not report["checks"]["stale_rows_preserved_ok"]:
        reasons.append("stale_rows_dropped")
