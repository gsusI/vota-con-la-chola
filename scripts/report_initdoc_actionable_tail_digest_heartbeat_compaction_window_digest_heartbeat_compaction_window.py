#!/usr/bin/env python3
"""Parity report for raw vs compacted initdoc compact-window-digest heartbeat (last-N window)."""
...
try:
    ...
    from report_initdoc_actionable_tail_digest_heartbeat_compaction_window import (
        DEFAULT_LAST,
        build_compaction_window_report_from_paths,
    )
...
def parse_args(argv: list[str] | None = None) -> argparse.Namespace:
    p = argparse.ArgumentParser(description="Initdoc compact-window-digest heartbeat compaction-window report")
    p.add_argument(
        "--heartbeat-jsonl",
