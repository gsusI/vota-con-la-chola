#!/usr/bin/env python3
"""Parity report for raw vs compacted trust-action nudges heartbeat (last-N window)."""
...
def parse_args(argv: list[str] | None = None) -> argparse.Namespace:
    p = argparse.ArgumentParser(description="Citizen trust-action nudges heartbeat compaction-window report")
    p.add_argument(
        "--heartbeat-jsonl",
...
def _to_ref(row: dict[str, Any]) -> dict[str, Any]:
    ...
    return {
        ...
        "nudge_clickthrough_violation": _is_threshold_violation(entry, key_ok="nudge_clickthrough_meets_minimum"),
...
def _has_incident(ref: dict[str, Any]) -> bool:
    ...
    if not bool(ref.get("contract_complete")):
        return True
    if bool(ref.get("nudge_clickthrough_violation")):
        return True
    return False
...
def _sample_ref(ref: dict[str, Any], *, present: bool) -> dict[str, Any]:
    return {
        ...
        "nudge_clickthrough_violation": bool(ref.get("nudge_clickthrough_violation")),
...
def build_compaction_window_report(
    ...
    *,
    ...
    report: dict[str, Any] = {
        ...
        "raw_window_nudge_clickthrough_violations": 0,
        ...
        "nudge_clickthrough_violations_present_in_compacted": 0,
        "nudge_clickthrough_violations_missing_in_compacted": 0,
        ...
        "checks": {
            ...
            "nudge_clickthrough_violations_parity_ok": False,
    ...
    for ref in raw_window_refs:
        ...
        is_nudge_clickthrough_violation = bool(ref.get("nudge_clickthrough_violation"))
        ...
        if is_nudge_clickthrough_violation:
            report["raw_window_nudge_clickthrough_violations"] += 1
        ...
        if present:
            ...
            if is_nudge_clickthrough_violation:
                report["nudge_clickthrough_violations_present_in_compacted"] += 1
        else:
            ...
            if is_nudge_clickthrough_violation:
                report["nudge_clickthrough_violations_missing_in_compacted"] += 1
    ...
    checks["nudge_clickthrough_violations_parity_ok"] = (
        report["nudge_clickthrough_violations_present_in_compacted"]
        >= report["raw_window_nudge_clickthrough_violations"]
    )
    ...
    if not checks["nudge_clickthrough_violations_parity_ok"]:
        reasons.append("nudge_clickthrough_violations_underreported_in_compacted")
