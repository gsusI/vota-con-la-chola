...
function usage() {
  return [
    "Usage:",
    "  node scripts/report_citizen_preset_contract_bundle_history_slo_digest_heartbeat_window.js [options]",
    "",
    "Options:",
...
function computeFailedLatestStreak(rows) {
  let streak = 0;
  for (let i = rows.length - 1; i >= 0; i -= 1) {
    const row = rows[i];
    if (!row || row.malformed_line) break;
    if (normalizeStatus(safeObj(row.entry).status) === "failed") {
      streak += 1;
      continue;
    }
    break;
...
function main() {
  ...
  try {
    ...
    if (report.entries_in_window > 0) {
      ...
      report.failed_streak_latest = computeFailedLatestStreak(windowRows);
    }
