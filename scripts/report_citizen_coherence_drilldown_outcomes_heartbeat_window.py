#!/usr/bin/env python3
"""Windowed trend report for citizen coherence drilldown outcomes heartbeat JSONL."""
...
DEFAULT_MAX_REPLAY_SUCCESS_RATE_VIOLATIONS = 0
DEFAULT_MAX_REPLAY_SUCCESS_RATE_VIOLATION_RATE_PCT = 0.0
DEFAULT_MAX_CONTRACT_CLICK_RATE_VIOLATIONS = 0
DEFAULT_MAX_CONTRACT_CLICK_RATE_VIOLATION_RATE_PCT = 0.0
DEFAULT_MAX_REPLAY_FAILURE_RATE_VIOLATIONS = 0
DEFAULT_MAX_REPLAY_FAILURE_RATE_VIOLATION_RATE_PCT = 0.0
...
def parse_args(argv: list[str] | None = None) -> argparse.Namespace:
    p = argparse.ArgumentParser(description="Citizen coherence drilldown outcomes heartbeat window report")
    p.add_argument(
        "--heartbeat-jsonl",
    ...
    p.add_argument(
        ...
        type=int,
        default=DEFAULT_MAX_REPLAY_SUCCESS_RATE_VIOLATIONS,
    )
    p.add_argument(
        "--max-replay-success-rate-violation-rate-pct",
        type=float,
        default=DEFAULT_MAX_REPLAY_SUCCESS_RATE_VIOLATION_RATE_PCT,
    )
    p.add_argument(
        "--max-contract-click-rate-violations",
        type=int,
        default=DEFAULT_MAX_CONTRACT_CLICK_RATE_VIOLATIONS,
    )
    p.add_argument(
        "--max-contract-click-rate-violation-rate-pct",
        type=float,
        default=DEFAULT_MAX_CONTRACT_CLICK_RATE_VIOLATION_RATE_PCT,
    )
    p.add_argument(
        "--max-replay-failure-rate-violations",
        type=int,
        default=DEFAULT_MAX_REPLAY_FAILURE_RATE_VIOLATIONS,
    )
    p.add_argument(
        "--max-replay-failure-rate-violation-rate-pct",
        type=float,
        default=DEFAULT_MAX_REPLAY_FAILURE_RATE_VIOLATION_RATE_PCT,
    )
...
def _latest_summary(row: dict[str, Any] | None) -> dict[str, Any]:
    if not row:
        return {
            ...
            "replay_failure_rate": None,
            "contract_complete_click_rate": None,
            "contract_complete": None,
            "replay_success_rate_meets_minimum": None,
            "contract_complete_click_rate_meets_minimum": None,
            "replay_failure_rate_within_threshold": None,
        }
    ...
    return {
        ...
        "replay_success_rate": _safe_float(entry.get("replay_success_rate")),
        "replay_failure_rate": _safe_float(entry.get("replay_failure_rate")),
        "contract_complete_click_rate": _safe_float(entry.get("contract_complete_click_rate")),
        "contract_complete": bool(entry.get("contract_complete")) if "contract_complete" in entry else None,
        "replay_success_rate_meets_minimum": entry.get("replay_success_rate_meets_minimum")
        if isinstance(entry.get("replay_success_rate_meets_minimum"), bool)
        else None,
        "contract_complete_click_rate_meets_minimum": entry.get("contract_complete_click_rate_meets_minimum")
        if isinstance(entry.get("contract_complete_click_rate_meets_minimum"), bool)
        else None,
        "replay_failure_rate_within_threshold": entry.get("replay_failure_rate_within_threshold")
        if isinstance(entry.get("replay_failure_rate_within_threshold"), bool)
        else None,
    }
...
def build_window_report(
    ...
    *,
    ...
    max_replay_success_rate_violations: int = DEFAULT_MAX_REPLAY_SUCCESS_RATE_VIOLATIONS,
    max_replay_success_rate_violation_rate_pct: float = DEFAULT_MAX_REPLAY_SUCCESS_RATE_VIOLATION_RATE_PCT,
    max_contract_click_rate_violations: int = DEFAULT_MAX_CONTRACT_CLICK_RATE_VIOLATIONS,
    max_contract_click_rate_violation_rate_pct: float = DEFAULT_MAX_CONTRACT_CLICK_RATE_VIOLATION_RATE_PCT,
    max_replay_failure_rate_violations: int = DEFAULT_MAX_REPLAY_FAILURE_RATE_VIOLATIONS,
    max_replay_failure_rate_violation_rate_pct: float = DEFAULT_MAX_REPLAY_FAILURE_RATE_VIOLATION_RATE_PCT,
    strict: bool = False,
    heartbeat_path: str = "",
) -> dict[str, Any]:
    ...
    max_replay_success_viol_n = _parse_non_negative_int(
        max_replay_success_rate_violations,
        arg_name="max_replay_success_rate_violations",
    )
    max_replay_success_viol_rate = _parse_non_negative_float(
        max_replay_success_rate_violation_rate_pct,
        arg_name="max_replay_success_rate_violation_rate_pct",
    )
    max_contract_click_viol_n = _parse_non_negative_int(
        max_contract_click_rate_violations,
        arg_name="max_contract_click_rate_violations",
    )
    max_contract_click_viol_rate = _parse_non_negative_float(
        max_contract_click_rate_violation_rate_pct,
        arg_name="max_contract_click_rate_violation_rate_pct",
    )
    max_replay_failure_viol_n = _parse_non_negative_int(
        max_replay_failure_rate_violations,
        arg_name="max_replay_failure_rate_violations",
    )
    max_replay_failure_viol_rate = _parse_non_negative_float(
        max_replay_failure_rate_violation_rate_pct,
        arg_name="max_replay_failure_rate_violation_rate_pct",
    )
    ...
    report: dict[str, Any] = {
        ...
        "thresholds": {
            ...
            "max_replay_success_rate_violations": int(max_replay_success_viol_n),
            "max_replay_success_rate_violation_rate_pct": float(max_replay_success_viol_rate),
            "max_contract_click_rate_violations": int(max_contract_click_viol_n),
            "max_contract_click_rate_violation_rate_pct": float(max_contract_click_viol_rate),
            "max_replay_failure_rate_violations": int(max_replay_failure_viol_n),
            "max_replay_failure_rate_violation_rate_pct": float(max_replay_failure_viol_rate),
        },
    ...
    for row in window_rows:
        ...
        if _is_threshold_violation(entry, key_ok="replay_success_rate_meets_minimum"):
            report["replay_success_rate_violations_in_window"] = int(report["replay_success_rate_violations_in_window"]) + 1
        if _is_threshold_violation(entry, key_ok="contract_complete_click_rate_meets_minimum"):
            report["contract_click_rate_violations_in_window"] = int(report["contract_click_rate_violations_in_window"]) + 1
        if _is_threshold_violation(entry, key_ok="replay_failure_rate_within_threshold"):
            report["replay_failure_rate_violations_in_window"] = int(report["replay_failure_rate_violations_in_window"]) + 1
    ...
    if report["entries_in_window"] > 0:
        ...
        report["replay_success_rate_violation_rate_pct"] = _rate_pct(
            int(report["replay_success_rate_violations_in_window"]),
            denom,
        )
        report["contract_click_rate_violation_rate_pct"] = _rate_pct(
            int(report["contract_click_rate_violations_in_window"]),
            denom,
        )
        report["replay_failure_rate_violation_rate_pct"] = _rate_pct(
            int(report["replay_failure_rate_violations_in_window"]),
            denom,
        )
    ...
    checks["max_replay_success_rate_violations_ok"] = (
        report["replay_success_rate_violations_in_window"] <= max_replay_success_viol_n
    )
    checks["max_replay_success_rate_violation_rate_ok"] = (
        float(report["replay_success_rate_violation_rate_pct"]) <= max_replay_success_viol_rate
    )
    checks["max_contract_click_rate_violations_ok"] = (
        report["contract_click_rate_violations_in_window"] <= max_contract_click_viol_n
    )
    checks["max_contract_click_rate_violation_rate_ok"] = (
        float(report["contract_click_rate_violation_rate_pct"]) <= max_contract_click_viol_rate
    )
    checks["max_replay_failure_rate_violations_ok"] = (
        report["replay_failure_rate_violations_in_window"] <= max_replay_failure_viol_n
    )
    checks["max_replay_failure_rate_violation_rate_ok"] = (
        float(report["replay_failure_rate_violation_rate_pct"]) <= max_replay_failure_viol_rate
    )
    ...
    checks["latest_thresholds_ok"] = (
        report["entries_in_window"] > 0
        and not bool(_safe_obj(report["latest"]).get("malformed_line"))
        and bool(_safe_obj(report["latest"]).get("replay_success_rate_meets_minimum"))
        and bool(_safe_obj(report["latest"]).get("contract_complete_click_rate_meets_minimum"))
        and bool(_safe_obj(report["latest"]).get("replay_failure_rate_within_threshold"))
    )
    ...
    if not checks["max_replay_success_rate_violations_ok"]:
        reasons.append("max_replay_success_rate_violations_exceeded")
    if not checks["max_replay_success_rate_violation_rate_ok"]:
        reasons.append("max_replay_success_rate_violation_rate_exceeded")
    if not checks["max_contract_click_rate_violations_ok"]:
        reasons.append("max_contract_click_rate_violations_exceeded")
    if not checks["max_contract_click_rate_violation_rate_ok"]:
        reasons.append("max_contract_click_rate_violation_rate_exceeded")
    if not checks["max_replay_failure_rate_violations_ok"]:
        reasons.append("max_replay_failure_rate_violations_exceeded")
    if not checks["max_replay_failure_rate_violation_rate_ok"]:
        reasons.append("max_replay_failure_rate_violation_rate_exceeded")
    ...
    if report["entries_in_window"] == 0:
        ...
        or report["replay_success_rate_violations_in_window"] > 0
        or report["contract_click_rate_violations_in_window"] > 0
        or report["replay_failure_rate_violations_in_window"] > 0
    ):
        report["status"] = "degraded"
    else:
        report["status"] = "ok"
...
def build_window_report_from_path(
    ...
    *,
    ...
    return build_window_report(
        ...
        max_replay_success_rate_violations=max_replay_success_rate_violations,
        max_replay_success_rate_violation_rate_pct=max_replay_success_rate_violation_rate_pct,
        max_contract_click_rate_violations=max_contract_click_rate_violations,
        max_contract_click_rate_violation_rate_pct=max_contract_click_rate_violation_rate_pct,
        max_replay_failure_rate_violations=max_replay_failure_rate_violations,
        max_replay_failure_rate_violation_rate_pct=max_replay_failure_rate_violation_rate_pct,
        strict=strict,
...
def main(argv: list[str] | None = None) -> int:
    ...
    try:
        report = build_window_report_from_path(
            ...
            max_replay_success_rate_violations=int(args.max_replay_success_rate_violations),
            max_replay_success_rate_violation_rate_pct=float(args.max_replay_success_rate_violation_rate_pct),
            max_contract_click_rate_violations=int(args.max_contract_click_rate_violations),
            max_contract_click_rate_violation_rate_pct=float(args.max_contract_click_rate_violation_rate_pct),
            max_replay_failure_rate_violations=int(args.max_replay_failure_rate_violations),
            max_replay_failure_rate_violation_rate_pct=float(args.max_replay_failure_rate_violation_rate_pct),
