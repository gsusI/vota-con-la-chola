#!/usr/bin/env python3
"""Parity report for raw vs compacted citizen release-trace digest heartbeat (last-N window)."""
...
DEFAULT_COMPACTED_JSONL = Path("docs/etl/runs/citizen_release_trace_digest_heartbeat.compacted.jsonl")
DEFAULT_LAST = 20
...
def parse_args(argv: list[str] | None = None) -> argparse.Namespace:
    p = argparse.ArgumentParser(description="Citizen release-trace digest heartbeat compaction-window report")
    p.add_argument(
        "--heartbeat-jsonl",
...
def _stale_detected(entry: dict[str, Any]) -> bool | None:
    ...
    degraded_reasons = set(_safe_list_str(entry.get("degraded_reasons")))
    if "release_trace_stale" in degraded_reasons:
        return True
    return None
...
def _has_incident(ref: dict[str, Any]) -> bool:
    ...
    if len(_safe_list_str(ref.get("strict_fail_reasons"))) > 0:
        return True
    if ref.get("stale_detected") is True:
        return True
    return False
...
def _sample_ref(ref: dict[str, Any], *, present: bool) -> dict[str, Any]:
    return {
        ...
        "freshness_within_sla": ref.get("freshness_within_sla"),
        "stale_detected": ref.get("stale_detected"),
...
def build_compaction_window_report(
    ...
    *,
    ...
    report: dict[str, Any] = {
        ...
        "stale_rows_present_in_compacted": 0,
        "stale_rows_missing_in_compacted": 0,
    ...
    for ref in raw_window_refs:
        ...
        is_stale_row = ref.get("stale_detected") is True
        ...
        if is_stale_row:
            report["raw_window_stale_rows"] += 1
        ...
        if present:
            ...
            if is_stale_row:
                report["stale_rows_present_in_compacted"] += 1
        else:
            ...
            if is_stale_row:
                report["stale_rows_missing_in_compacted"] += 1
    ...
    checks["stale_rows_parity_ok"] = report["stale_rows_present_in_compacted"] >= report["raw_window_stale_rows"]
    ...
    if not checks["stale_rows_parity_ok"]:
        reasons.append("stale_rows_underreported_in_compacted")
