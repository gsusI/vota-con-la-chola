...
function usage() {
  return [
    "Usage:",
    "  node scripts/report_citizen_preset_contract_bundle_history_slo_digest_heartbeat_compaction_window_digest_heartbeat_compaction_window.js [options]",
    "",
    "Options:",
    "  --heartbeat-jsonl <path>   Raw heartbeat JSONL path (default: docs/etl/runs/citizen_preset_contract_bundle_history_slo_digest_heartbeat_compaction_window_digest_heartbeat.jsonl)",
    "  --compacted-jsonl <path>   Compacted heartbeat JSONL path (default: docs/etl/runs/citizen_preset_contract_bundle_history_slo_digest_heartbeat_compaction_window_digest_heartbeat.compacted.jsonl)",
...
function parseArgs(argv) {
  const out = {
    ...
    compactedJsonl:
      "docs/etl/runs/citizen_preset_contract_bundle_history_slo_digest_heartbeat_compaction_window_digest_heartbeat.compacted.jsonl",
    last: 20,
    jsonOut: "",
    strict: false,
    help: false,
...
function hasIncident(ref) {
  ...
  if (ref.status === "degraded") return true;
...
function main() {
  ...
  try {
    ...
    for (const ref of rawWindowRefs) {
      ...
      if (ref.status === "degraded") report.raw_window_degraded += 1;
      ...
      if (present) {
        ...
        if (ref.status === "degraded") report.degraded_present_in_compacted += 1;
    ...
    report.checks.degraded_parity_ok = report.degraded_present_in_compacted === report.raw_window_degraded;
  ...
  if (!toBool(report.checks.degraded_parity_ok)) {
    report.strict_fail_reasons.push("degraded_count_underreported_in_compacted_window");
  }
