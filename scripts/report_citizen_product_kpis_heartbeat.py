#!/usr/bin/env python3
"""Append-only heartbeat lane for citizen product KPI digest."""
...
DEFAULT_DIGEST_JSON = Path("docs/etl/sprints/AI-OPS-98/evidence/citizen_product_kpis_latest.json")
...
def parse_args(argv: list[str] | None = None) -> argparse.Namespace:
    p = argparse.ArgumentParser(description="Append heartbeat JSONL for citizen product KPI digest")
    p.add_argument(
        "--digest-json",
        default=str(DEFAULT_DIGEST_JSON),
        help=f"Input product KPI digest JSON (default: {DEFAULT_DIGEST_JSON})",
    )
    p.add_argument(
        "--heartbeat-jsonl",
...
def build_heartbeat(
    ...
    *,
    ...
    time_to_first_answer_seconds = _safe_float(metrics.get("time_to_first_answer_seconds"))
    drilldown_click_rate = _safe_float(metrics.get("drilldown_click_rate"))

    max_unknown_rate = _safe_float(thresholds.get("max_unknown_rate"))
    max_tfa_seconds = _safe_float(thresholds.get("max_time_to_first_answer_seconds"))
    min_drill_rate = _safe_float(thresholds.get("min_drilldown_click_rate"))

    unknown_ok_raw = checks.get("unknown_rate_within_threshold")
    tfa_ok_raw = checks.get("time_to_first_answer_within_threshold")
    drill_ok_raw = checks.get("drilldown_click_rate_within_threshold")

    unknown_ok = bool(unknown_ok_raw) if isinstance(unknown_ok_raw, bool) else None
    tfa_ok = bool(tfa_ok_raw) if isinstance(tfa_ok_raw, bool) else None
    drill_ok = bool(drill_ok_raw) if isinstance(drill_ok_raw, bool) else None

    telemetry_available = bool(checks.get("telemetry_available"))
    contract_complete = bool(checks.get("contract_complete"))

    missing_metrics = _safe_list_str(digest.get("missing_metrics"))
    ...
    if status == "failed":
        ...
        if not failure_reasons:
            strict_fail_reasons.append("product_kpis_failed_without_reason")
    ...
    heartbeat_id = "|".join(
        [
            ...
            status,
            digest_generated_at,
            _fmt_float(unknown_rate),
            _fmt_float(time_to_first_answer_seconds),
            _fmt_float(drilldown_click_rate),
            _fmt_float(max_unknown_rate),
            _fmt_float(max_tfa_seconds),
            _fmt_float(min_drill_rate),
    ...
    return {
        ...
        "time_to_first_answer_seconds": round(float(time_to_first_answer_seconds), 6)
        if time_to_first_answer_seconds is not None
        else None,
        "drilldown_click_rate": round(float(drilldown_click_rate), 6) if drilldown_click_rate is not None else None,
        "max_unknown_rate": round(float(max_unknown_rate), 6) if max_unknown_rate is not None else None,
        "max_time_to_first_answer_seconds": round(float(max_tfa_seconds), 6) if max_tfa_seconds is not None else None,
        "min_drilldown_click_rate": round(float(min_drill_rate), 6) if min_drill_rate is not None else None,
...
def validate_heartbeat(heartbeat: dict[str, Any]) -> list[str]:
    ...
    unknown_rate = _safe_float(heartbeat.get("unknown_rate"))
    if unknown_rate is not None and not (0.0 <= unknown_rate <= 1.0):
        reasons.append("invalid_unknown_rate")

    drill_rate = _safe_float(heartbeat.get("drilldown_click_rate"))
    if drill_rate is not None and not (0.0 <= drill_rate <= 1.0):
        reasons.append("invalid_drilldown_click_rate")

    tfa_seconds = _safe_float(heartbeat.get("time_to_first_answer_seconds"))
    if tfa_seconds is not None and tfa_seconds < 0.0:
        reasons.append("invalid_time_to_first_answer_seconds")

    max_unknown_rate = _safe_float(heartbeat.get("max_unknown_rate"))
    if max_unknown_rate is not None and not (0.0 <= max_unknown_rate <= 1.0):
        reasons.append("invalid_max_unknown_rate")

    max_tfa_seconds = _safe_float(heartbeat.get("max_time_to_first_answer_seconds"))
    if max_tfa_seconds is not None and max_tfa_seconds <= 0.0:
        reasons.append("invalid_max_time_to_first_answer_seconds")

    min_drill_rate = _safe_float(heartbeat.get("min_drilldown_click_rate"))
    if min_drill_rate is not None and not (0.0 <= min_drill_rate <= 1.0):
        reasons.append("invalid_min_drilldown_click_rate")
