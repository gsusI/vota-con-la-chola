#!/usr/bin/env python3
"""Windowed trend report for citizen release-trace digest heartbeat JSONL."""
...
DEFAULT_MAX_STALE = 0
DEFAULT_MAX_STALE_RATE_PCT = 0.0
...
def _stale_detected(entry: dict[str, Any]) -> bool | None:
    raw = entry.get("stale_detected")
    if isinstance(raw, bool):
        return raw
    freshness_raw = entry.get("freshness_within_sla")
    if isinstance(freshness_raw, bool):
        return not freshness_raw
    degraded_reasons = _safe_list_str(entry.get("degraded_reasons"))
    if "release_trace_stale" in set(degraded_reasons):
        return True
    return None


def parse_args(argv: list[str] | None = None) -> argparse.Namespace:
    p = argparse.ArgumentParser(description="Citizen release-trace digest heartbeat window report")
    p.add_argument(
        "--heartbeat-jsonl",
    ...
    p.add_argument(
        "--max-stale",
        type=int,
        default=DEFAULT_MAX_STALE,
        help=f"Max allowed stale rows in window (default: {DEFAULT_MAX_STALE})",
    )
    p.add_argument(
        "--max-stale-rate-pct",
        type=float,
        default=DEFAULT_MAX_STALE_RATE_PCT,
        help=f"Max allowed stale rate pct in window (default: {DEFAULT_MAX_STALE_RATE_PCT})",
    )
    p.add_argument(
        "--strict",
        action="store_true",
...
def _latest_summary(row: dict[str, Any] | None) -> dict[str, Any]:
    if not row:
        return {
            ...
            "release_age_minutes": None,
            "max_age_minutes": None,
            "freshness_within_sla": None,
            "stale_detected": None,
            "release_ready": None,
            "release_total_fail": None,
        }
    ...
    return {
        ...
        "release_generated_at": _safe_text(entry.get("release_generated_at")),
        "release_age_minutes": _safe_float(entry.get("release_age_minutes")),
        "max_age_minutes": _safe_float(entry.get("max_age_minutes")),
        "freshness_within_sla": entry.get("freshness_within_sla") if isinstance(entry.get("freshness_within_sla"), bool) else None,
        "stale_detected": _stale_detected(entry),
        "release_ready": bool(entry.get("release_ready")) if "release_ready" in entry else None,
        "release_total_fail": _to_int(entry.get("release_total_fail"), 0) if "release_total_fail" in entry else None,
    }
...
def build_window_report(
    ...
    *,
    ...
    max_stale_rate_pct: float = DEFAULT_MAX_STALE_RATE_PCT,
    strict: bool = False,
    heartbeat_path: str = "",
) -> dict[str, Any]:
    ...
    max_stale_n = _parse_non_negative_int(max_stale, arg_name="max_stale")
    max_stale_rate = _parse_non_negative_float(max_stale_rate_pct, arg_name="max_stale_rate_pct")
    ...
    report: dict[str, Any] = {
        ...
        "thresholds": {
            ...
            "max_stale": int(max_stale_n),
            "max_stale_rate_pct": float(max_stale_rate),
        },
    ...
    for row in window_rows:
        ...
        stale = _stale_detected(entry)
        if stale is True:
            report["stale_in_window"] = int(report["stale_in_window"]) + 1
            if not report["first_stale_run_at"]:
                report["first_stale_run_at"] = run_at
            report["last_stale_run_at"] = run_at
    ...
    if report["entries_in_window"] > 0:
        ...
        report["stale_rate_pct"] = _round4((float(report["stale_in_window"]) / denom) * 100.0)
    ...
    checks["max_stale_ok"] = report["stale_in_window"] <= max_stale_n
    checks["max_stale_rate_ok"] = float(report["stale_rate_pct"]) <= max_stale_rate
    ...
    latest_stale = _safe_obj(report["latest"]).get("stale_detected")
    checks["latest_freshness_within_sla_ok"] = (
        report["entries_in_window"] > 0
        and not bool(_safe_obj(report["latest"]).get("malformed_line"))
        and latest_stale is not True
    )
    ...
    if not checks["max_stale_ok"]:
        reasons.append("max_stale_exceeded")
    if not checks["max_stale_rate_ok"]:
        reasons.append("max_stale_rate_exceeded")
    if not checks["latest_not_failed_ok"]:
        reasons.append("latest_status_failed")
    if not checks["latest_freshness_within_sla_ok"]:
        reasons.append("latest_release_trace_stale")
    ...
    if report["entries_in_window"] == 0:
        ...
        or not checks["max_stale_ok"]
        or not checks["max_stale_rate_ok"]
        or not checks["latest_not_failed_ok"]
        or not checks["latest_freshness_within_sla_ok"]
    ):
        report["status"] = "failed"
    elif not checks["max_degraded_ok"] or not checks["max_degraded_rate_ok"]:
        report["status"] = "failed"
    elif report["degraded_in_window"] > 0 or report["stale_in_window"] > 0:
        report["status"] = "degraded"
    else:
        report["status"] = "ok"
...
def build_window_report_from_path(
    ...
    *,
    ...
    return build_window_report(
        ...
        max_stale=max_stale,
        max_stale_rate_pct=max_stale_rate_pct,
        strict=strict,
...
def main(argv: list[str] | None = None) -> int:
    ...
    try:
        report = build_window_report_from_path(
            ...
            max_stale_rate_pct=float(args.max_stale_rate_pct),
