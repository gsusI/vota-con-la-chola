#!/usr/bin/env python3
"""Compact digest for packet-fix queue heartbeat compaction-window parity."""
...
DEFAULT_COMPACTION_WINDOW_JSON = Path(
    "docs/etl/sprints/AI-OPS-195/evidence/"
    "sanction_procedural_official_review_packet_fix_queue_heartbeat_compaction_window_latest.json"
)
...
def parse_args(argv: list[str] | None = None) -> argparse.Namespace:
    p = argparse.ArgumentParser(description="Packet-fix queue heartbeat compaction-window digest report")
    p.add_argument(
        ...
        help=(
            ...
            f"(default: {DEFAULT_COMPACTION_WINDOW_JSON})"
        ),
    )
    p.add_argument("--strict", action="store_true", help="Exit with code 4 when digest status is failed")
