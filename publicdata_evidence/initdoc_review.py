...
from publicdata_core.util import normalize_ws, now_utc_iso, stable_json
...
def export_label_studio_tasks(rows: list[sqlite3.Row | Mapping[str, Any]]) -> list[dict[str, Any]]:
    ...
    for row in rows:
        ...
        source_link_html = (
            f'<a class="initdoc-review__source-link" href="{source_url}" target="_blank" rel="noopener noreferrer">Abrir documento fuente</a>'
            if source_url
            else '<p class="initdoc-review__missing-source">Sin URL fuente</p>'
        )
        task = {
            "data": {
