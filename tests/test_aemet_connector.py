...
from etl.politicos_es.connectors.aemet_indicators import (
...
class TestAemetConnector(unittest.TestCase):
    def test_extract_from_sample_json_includes_station_metadata(self) -> None:
        ...
        with tempfile.TemporaryDirectory() as td:
            ...
            self.assertTrue(str(first.get("station_name") or "").strip())
    ...
    def test_parser_accepts_serialized_records_container(self) -> None:
        ...
        baseline = parse_aemet_records(
            payload,
            feed_url="https://opendata.aemet.es/opendata/api/valores/climatologicos/diarios/datos",
            content_type="application/json",
        )
        wrapped_payload = json.dumps(
            {
                "source": "aemet_opendata_series_network",
                "feed_url": "https://opendata.aemet.es/opendata/api/valores/climatologicos/diarios/datos",
                "records": baseline,
            },
            ensure_ascii=True,
        ...
        replay_records = parse_aemet_records(
            wrapped_payload,
            feed_url="file:///tmp/aemet-replay.json",
            content_type="application/json",
        )
    ...
    def test_parser_rejects_metric_value_snapshot_payload(self) -> None:
        ...
        with self.assertRaisesRegex(RuntimeError, "metric,value"):
            parse_aemet_records(
                snapshot_payload,
                feed_url="file:///tmp/aemet-run-snapshot.csv",
                content_type="text/csv",
            )

    def test_token_placeholder_requires_env_var(self) -> None:
        connector = AemetOpenDataSeriesConnector()
        with mock.patch.dict("os.environ", {}, clear=False):
            with self.assertRaisesRegex(RuntimeError, "AEMET_API_KEY"):
                connector.resolve_url("https://opendata.aemet.es/api?api_key={api_key}", timeout=5)

    def test_strict_network_http_404_is_classified_as_contract(self) -> None:
        connector = AemetOpenDataSeriesConnector()
        with tempfile.TemporaryDirectory() as td:
            raw_dir = Path(td) / "raw"
            ...
            with mock.patch("etl.politicos_es.connectors.aemet_indicators.http_get_bytes", side_effect=http_error):
                with self.assertRaisesRegex(RuntimeError, "aemet_blocker=contract"):
                    connector.extract(
                        ...
                        timeout=5,
                        from_file=None,
                        url_override="https://opendata.aemet.es/opendata/api/valores/climatologicos",
                        strict_network=True,
                    )
    ...
    def test_source_records_ingest_is_idempotent(self) -> None:
        ...
        with tempfile.TemporaryDirectory() as td:
            ...
            try:
                ...
                count_1 = int(
                    conn.execute(
                        "SELECT COUNT(*) AS c FROM source_records WHERE source_id='aemet_opendata_series'"
                    ).fetchone()["c"]
                )
