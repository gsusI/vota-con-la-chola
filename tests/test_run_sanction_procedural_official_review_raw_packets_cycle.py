...
from scripts.run_sanction_procedural_official_review_raw_packets_cycle import (
    main as packets_cycle_main,
)


class TestRunSanctionProceduralOfficialReviewRawPacketsCycle(unittest.TestCase):
    ...
    def _load_packet_plan(self, db_path: Path, statuses: set[str]) -> list[dict[str, object]]:
        conn = open_db(db_path)
        try:
    ...
    def _run_packets_cycle(self, argv: list[str]) -> int:
        import sys
        ...
        try:
            ...
            with redirect_stdout(io.StringIO()), redirect_stderr(io.StringIO()):
                return int(packets_cycle_main())
        finally:
            sys.argv = old

    def test_strict_packet_coverage_blocks_on_missing_files(self) -> None:
        with TemporaryDirectory() as td:
            db_path = Path(td) / "raw_packets_cycle_strict.db"
            packets_dir = Path(td) / "packets"
            out_path = Path(td) / "out.json"
            self._setup_db(db_path)
            packets = self._load_packet_plan(db_path, statuses={"missing_metric"})

            packet = dict(packets[0])
            row = dict(packet["row"])
            self._write_packet(packets_dir / str(packet["packet_filename"]), row)

            rc = self._run_packets_cycle(
                [
                    "run_sanction_procedural_official_review_raw_packets_cycle.py",
                    "--db",
                    str(db_path),
                    "--packets-dir",
                    str(packets_dir),
        ...
        self.assertEqual(str(payload["packet_input"]["status"]), "degraded")
        self.assertEqual(int(payload["packet_input"]["totals"]["packets_expected_total"]), 4)
        self.assertEqual(int(payload["packet_input"]["totals"]["packet_files_missing_total"]), 3)
        self.assertEqual(str(payload["cycle"]["cycle"]["apply"]["skip_reason"]), "missing_packet_files")

    def test_full_packets_run_dry_cycle_ok(self) -> None:
        with TemporaryDirectory() as td:
            db_path = Path(td) / "raw_packets_cycle_ok.db"
            packets_dir = Path(td) / "packets"
            out_path = Path(td) / "out.json"
            self._setup_db(db_path)
            packets = self._load_packet_plan(db_path, statuses={"missing_metric"})
        ...
        self.assertEqual(str(payload["packet_input"]["status"]), "ok")
        self.assertEqual(int(payload["packet_input"]["totals"]["packet_rows_loaded_total"]), 4)
        self.assertEqual(str(payload["cycle"]["raw"]["status"]), "ok")
        self.assertEqual(int(payload["cycle"]["raw"]["totals"]["kpi_rows_emitted"]), 12)
        self.assertEqual(str(payload["cycle"]["prepare"]["status"]), "ok")
        self.assertEqual(str(payload["cycle"]["cycle"]["readiness"]["status"]), "ok")
        self.assertEqual(int(payload["cycle"]["cycle"]["apply"]["counts"]["rows_ready"]), 12)
        self.assertEqual(n, 0)

    def test_strict_actionable_fails_when_status_filter_has_no_packets(self) -> None:
        with TemporaryDirectory() as td:
            db_path = Path(td) / "raw_packets_cycle_no_actionable.db"
            packets_dir = Path(td) / "packets"
            out_path = Path(td) / "out.json"
            self._setup_db(db_path)
        ...
        self.assertEqual(rc, 4)
        self.assertEqual(int(payload["packet_input"]["totals"]["packets_expected_total"]), 0)
        self.assertEqual(str(payload["cycle"]["cycle"]["apply"]["skip_reason"]), "no_actionable_packets")
