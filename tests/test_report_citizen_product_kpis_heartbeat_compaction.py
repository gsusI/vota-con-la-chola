...
from scripts.report_citizen_product_kpis_heartbeat_compaction import main


class TestCitizenProductKpisHeartbeatCompaction(unittest.TestCase):
    def _hb(
        self,
        minute: int,
        *,
    ...
    def test_compaction_preserves_incidents_and_threshold_violations(self) -> None:
        with tempfile.TemporaryDirectory() as td:
            ...
            self.assertEqual(int(report["unknown_rate_violations_total"]), 2)
            self.assertEqual(int(report["unknown_rate_violations_dropped"]), 0)
            self.assertEqual(int(report["tfa_violations_total"]), 2)
            self.assertEqual(int(report["tfa_violations_dropped"]), 0)
            self.assertEqual(int(report["drilldown_violations_total"]), 2)
            self.assertEqual(int(report["drilldown_violations_dropped"]), 0)
