...
from scripts.report_citizen_release_trace_digest_heartbeat_compaction import main


class TestCitizenReleaseTraceDigestHeartbeatCompaction(unittest.TestCase):
    def _hb(
        self,
        minute: int,
        *,
    ...
    def test_compaction_preserves_incidents_and_stale_rows(self) -> None:
        with tempfile.TemporaryDirectory() as td:
            ...
            rows[3] = self._hb(3, status="failed", strict_fail_count=1, freshness_within_sla=True, stale_detected=False)
            rows[9] = self._hb(9, status="degraded", freshness_within_sla=False, stale_detected=True)
            rows[12] = self._hb(12, status="ok", freshness_within_sla=False, stale_detected=True)
            ...
            self.assertEqual(int(report["stale_rows_total"]), 2)
            self.assertEqual(int(report["stale_rows_dropped"]), 0)
