...
function runParityReporter(args) {
  const script = path.join(
    ...
    "..",
    "scripts",
    "report_citizen_preset_contract_bundle_history_slo_digest_heartbeat_compaction_window.js"
  );
...
test("heartbeat compaction window parity reporter passes strict when compacted preserves raw last-N parity", () => {
  const tmpDir = fs.mkdtempSync(path.join(os.tmpdir(), "preset-hb-compaction-window-pass-"));
  const rawPath = path.join(tmpDir, "heartbeat.raw.jsonl");
  const compactedPath = path.join(tmpDir, "heartbeat.compacted.jsonl");

  writeJsonl(rawPath, [
    heartbeat(0),
    heartbeat(1, { status: "degraded", risk_level: "amber" }),
    heartbeat(2),
    heartbeat(3, {
      status: "failed",
      risk_level: "red",
  ...
  const out = parseStdoutJson(proc, "compaction_window_parity_pass");
...
test("heartbeat compaction window parity reporter fails strict when compacted misses incident rows", () => {
  const tmpDir = fs.mkdtempSync(path.join(os.tmpdir(), "preset-hb-compaction-window-incident-fail-"));
  const rawPath = path.join(tmpDir, "heartbeat.raw.jsonl");
  const compactedPath = path.join(tmpDir, "heartbeat.compacted.jsonl");
  ...
  const rawRows = [
    ...
    heartbeat(2, { status: "degraded", risk_level: "amber" }),
    heartbeat(3),
  ];
  ...
  const out = parseStdoutJson(proc, "compaction_window_parity_incident_fail");

  assert.equal(out.window_raw_entries, 4);
  assert.equal(out.incident_missing_in_compacted, 1);
  assert.equal(out.raw_window_failed, 1);
  assert.equal(out.failed_present_in_compacted, 0);
  assert.equal(out.raw_window_red, 1);
  assert.equal(out.red_present_in_compacted, 0);
  assert.equal(out.checks.latest_present_ok, true);
  assert.ok(out.strict_fail_reasons.includes("incident_rows_missing_in_compacted"));
  assert.ok(out.strict_fail_reasons.includes("failed_count_underreported_in_compacted_window"));
  assert.ok(out.strict_fail_reasons.includes("red_count_underreported_in_compacted_window"));
});

test("heartbeat compaction window parity reporter fails strict when latest raw entry is missing from compacted", () => {
  const tmpDir = fs.mkdtempSync(path.join(os.tmpdir(), "preset-hb-compaction-window-latest-fail-"));
  const rawPath = path.join(tmpDir, "heartbeat.raw.jsonl");
  const compactedPath = path.join(tmpDir, "heartbeat.compacted.jsonl");
  ...
  assert.equal(proc.status, 1, `expected strict failure; got ${proc.status}; stderr=${proc.stderr || ""}`);
  const out = parseStdoutJson(proc, "compaction_window_parity_latest_fail");
