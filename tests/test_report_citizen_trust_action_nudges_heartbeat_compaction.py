...
from scripts.report_citizen_trust_action_nudges_heartbeat_compaction import main


class TestCitizenTrustActionNudgesHeartbeatCompaction(unittest.TestCase):
    def _hb(
        self,
        minute: int,
        *,
        ...
        clickthrough_ok: bool = True,
    ) -> dict[str, object]:
        mm = f"{minute:02d}"
        strict_reasons = ["strict_fail"] if strict_fail_count > 0 else []
        return {
            "run_at": f"2026-02-23T16:{mm}:00+00:00",
    ...
    def test_compaction_preserves_incidents_and_threshold_violations(self) -> None:
        with tempfile.TemporaryDirectory() as td:
            ...
            rows[3] = self._hb(3, status="failed", strict_fail_count=1, contract_complete=False, clickthrough_ok=False)
            rows[9] = self._hb(9, status="degraded", contract_complete=False, clickthrough_ok=False)
            rows[12] = self._hb(12, status="ok", contract_complete=True, clickthrough_ok=False)
            rows[13] = self._hb(13, status="ok", contract_complete=True, clickthrough_ok=True)
            ...
            self.assertEqual(int(report["strict_rows_total"]), 1)
            self.assertEqual(int(report["strict_rows_dropped"]), 0)
            self.assertEqual(int(report["contract_incomplete_total"]), 2)
            self.assertEqual(int(report["contract_incomplete_dropped"]), 0)
            self.assertEqual(int(report["nudge_clickthrough_violations_total"]), 3)
            self.assertEqual(int(report["nudge_clickthrough_violations_dropped"]), 0)
