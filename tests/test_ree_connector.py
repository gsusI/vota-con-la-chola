...
from etl.politicos_es.connectors.ree_esios_indicators import ReeEsiosIndicatorsConnector, parse_ree_records
...
class TestReeEsiosConnector(unittest.TestCase):
    def test_extract_from_sample_json_includes_series_metadata(self) -> None:
        ...
        with tempfile.TemporaryDirectory() as td:
            ...
            self.assertEqual(str(first.get("record_kind") or ""), "ree_indicator_series")
            self.assertTrue(str(first.get("series_code") or "").strip())
            self.assertTrue(str(first.get("series_label") or "").strip())
    ...
    def test_parser_accepts_serialized_records_container(self) -> None:
        ...
        wrapped_payload = json.dumps(
            {
                "source": "ree_esios_indicators_network",
                "feed_url": "https://apidatos.ree.es/es/datos/demanda/evolucion",
                "records": baseline,
            },
            ensure_ascii=True,
        ...
        replay_records = parse_ree_records(
            wrapped_payload,
            feed_url="file:///tmp/ree-replay.json",
            content_type="application/json",
        )
    ...
    def test_parser_rejects_metric_value_snapshot_payload(self) -> None:
        ...
        with self.assertRaisesRegex(RuntimeError, "metric,value"):
            parse_ree_records(
                snapshot_payload,
                feed_url="file:///tmp/ree-run-snapshot.csv",
    ...
    def test_source_records_ingest_is_idempotent(self) -> None:
        ...
        with tempfile.TemporaryDirectory() as td:
            ...
            try:
                ...
                count_1 = int(
                    conn.execute(
                        "SELECT COUNT(*) AS c FROM source_records WHERE source_id='ree_esios_indicators'"
                    ).fetchone()["c"]
                )
