...
test("compact-window digest heartbeat compaction keeps degraded/failed/red incidents and writes compacted JSONL", () => {
  const tmpDir = fs.mkdtempSync(path.join(os.tmpdir(), "preset-compact-window-digest-hb-compaction-pass-"));
  ...
  const out = parseStdoutJson(proc, "compact_window_digest_hb_compaction_pass");

  assert.equal(out.entries_total, 16);
  assert.ok(out.selected_entries > 0);
  assert.ok(out.dropped_entries > 0);
  assert.equal(out.incidents_total, 3);
  assert.equal(out.incidents_selected, 3);
  assert.equal(out.incidents_dropped, 0);
  assert.equal(out.failed_total, 1);
  assert.equal(out.failed_selected, 1);
  assert.equal(out.degraded_total, 1);
  assert.equal(out.degraded_selected, 1);
  ...
  const degradedSample = out.selected_reasons_sample.find((x) => x.index === 3);
  assert.ok(degradedSample, "degraded incident entry missing from selected sample");
  assert.ok(degradedSample.reasons.includes("incident_entry"));
...
test("compact-window digest heartbeat compaction fails strict when no rows are dropped above threshold", () => {
  const tmpDir = fs.mkdtempSync(path.join(os.tmpdir(), "preset-compact-window-digest-hb-compaction-fail-"));
  const heartbeatPath = path.join(tmpDir, "heartbeat.jsonl");
  ...
  const out = parseStdoutJson(proc, "compact_window_digest_hb_compaction_fail");
