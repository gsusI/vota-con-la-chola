...
from scripts.report_initdoc_actionable_tail_digest_heartbeat_compaction import main


class TestInitdocActionableTailDigestHeartbeatCompaction(unittest.TestCase):
    def _hb(self, minute: int, *, status: str = "ok", strict_fail_count: int = 0) -> dict[str, object]:
        ...
        return {
            "run_at": f"2026-02-23T10:{mm}:00+00:00",
    ...
    def test_compaction_preserves_incidents_and_drops_when_expected(self) -> None:
        with tempfile.TemporaryDirectory() as td:
            ...
            rows[3] = self._hb(3, status="failed", strict_fail_count=1)
            rows[9] = self._hb(9, status="degraded")
