...
from scripts.report_initdoc_actionable_tail_digest_heartbeat_compaction_window import main


class TestInitdocActionableTailDigestHeartbeatCompactionWindow(unittest.TestCase):
    def _hb(self, minute: int, *, status: str = "ok", strict_fail_count: int = 0) -> dict[str, object]:
        mm = f"{minute:02d}"
        strict_reasons = ["strict_fail"] if strict_fail_count > 0 else []
        return {
            "run_at": f"2026-02-23T11:{mm}:00+00:00",
    ...
    def test_compaction_window_passes_when_incidents_and_latest_present(self) -> None:
        with tempfile.TemporaryDirectory() as td:
            ...
            raw_rows = [
                ...
                self._hb(1, status="failed", strict_fail_count=1),
                self._hb(2, status="degraded"),
                self._hb(3, status="ok"),
            ]
            compacted_rows = [
                self._hb(1, status="failed", strict_fail_count=1),
                self._hb(2, status="degraded"),
