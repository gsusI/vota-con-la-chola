...
from scripts.report_citizen_trust_action_nudges_heartbeat_window import main
...
def _heartbeat_entry(
    *,
    ...
    clickthrough_ok: bool,
) -> dict:
    run_at = f"2026-02-23T18:{idx:02d}:00+00:00"
    return {
        "run_at": run_at,
        "heartbeat_id": f"{run_at}|{status}|{idx}",
        "status": status,
        "digest_path": "docs/etl/sprints/AI-OPS-86/evidence/citizen_trust_action_nudges_latest.json",
        ...
        "failure_reasons": [] if status != "failed" else ["nudge_clickthrough_below_threshold"],
        "strict_fail_count": 0 if status != "failed" else 1,
        "strict_fail_reasons": [] if status != "failed" else ["nudge_clickthrough_below_threshold"],
    }


class TestReportCitizenTrustActionNudgesHeartbeatWindow(unittest.TestCase):
    def test_main_passes_strict_with_healthy_window(self) -> None:
        with tempfile.TemporaryDirectory() as td:
            ...
            _write_jsonl(
                heartbeat_jsonl,
                [
                    _heartbeat_entry(idx=1, status="ok", contract_complete=True, clickthrough_ok=True),
                    _heartbeat_entry(idx=2, status="ok", contract_complete=True, clickthrough_ok=True),
                    _heartbeat_entry(idx=3, status="ok", contract_complete=True, clickthrough_ok=True),
                ],
            )
            ...
            rc = main(
                [
                    ...
                    "0",
                    "--max-nudge-clickthrough-violations",
                    "0",
            ...
            self.assertEqual(int(report["nudge_clickthrough_violations_in_window"]), 0)
    ...
    def test_main_fails_strict_when_latest_has_threshold_violations(self) -> None:
        with tempfile.TemporaryDirectory() as td:
            ...
            _write_jsonl(
                ...
                [
                    _heartbeat_entry(idx=1, status="ok", contract_complete=True, clickthrough_ok=True),
                    _heartbeat_entry(idx=2, status="degraded", contract_complete=False, clickthrough_ok=False),
                ],
            )
            ...
            self.assertEqual(int(report["nudge_clickthrough_violations_in_window"]), 1)
            self.assertIn("max_contract_incomplete_exceeded", report["strict_fail_reasons"])
            self.assertIn("max_nudge_clickthrough_violations_exceeded", report["strict_fail_reasons"])
