...
from scripts.import_liberty_enforcement_seed import import_seed
...
class TestImportLibertyEnforcementSeed(unittest.TestCase):
    def test_import_is_idempotent(self) -> None:
        with TemporaryDirectory() as td:
            db_path = Path(td) / "enforcement_import.db"
            ...
            try:
                ...
                got1 = import_seed(conn, seed_doc=enforcement_seed_doc, source_id="", snapshot_date="2026-02-23")
                got2 = import_seed(conn, seed_doc=enforcement_seed_doc, source_id="", snapshot_date="2026-02-23")

                self.assertEqual(str(got1["status"]), "ok")
                self.assertEqual(int(got1["counts"]["observations_inserted"]), 16)
                self.assertEqual(int(got1["counts"]["unresolved_fragment_refs"]), 0)

                self.assertEqual(str(got2["status"]), "ok")
                self.assertEqual(int(got2["counts"]["observations_updated"]), 16)

                row = conn.execute("SELECT COUNT(*) AS n FROM liberty_enforcement_methodologies").fetchone()
                self.assertEqual(int(row["n"]), 1)
                row = conn.execute("SELECT COUNT(*) AS n FROM liberty_enforcement_observations").fetchone()
                self.assertEqual(int(row["n"]), 16)
            finally:
                conn.close()
