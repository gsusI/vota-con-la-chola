...
from scripts.report_citizen_concern_pack_outcomes_heartbeat_compaction_window import main


class TestCitizenConcernPackOutcomesHeartbeatCompactionWindow(unittest.TestCase):
    def _hb(
        self,
        minute: int,
        *,
    ...
    def test_compaction_window_passes_when_incidents_and_latest_present(self) -> None:
        with tempfile.TemporaryDirectory() as td:
            ...
            raw_rows = [
                ...
                self._hb(3, status="ok", followthrough_ok=False),
            ...
            self.assertEqual(int(report["weak_pack_followthrough_violations_missing_in_compacted"]), 0)
            self.assertEqual(bool(report["checks"]["latest_present_ok"]), True)
            self.assertEqual(list(report.get("strict_fail_reasons") or []), [])

    def test_compaction_window_strict_fails_when_followthrough_incident_missing(self) -> None:
        with tempfile.TemporaryDirectory() as td:
            td_path = Path(td)
            raw_path = td_path / "raw.jsonl"
            compacted_path = td_path / "compacted.jsonl"
            ...
            raw_rows = [
                self._hb(0, status="ok"),
                self._hb(1, status="ok", followthrough_ok=False),
            ...
            self.assertIn("weak_pack_followthrough_violations_underreported_in_compacted", reasons)
