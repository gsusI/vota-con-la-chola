...
from scripts.report_citizen_trust_action_nudges_heartbeat_compaction_window import main


class TestCitizenTrustActionNudgesHeartbeatCompactionWindow(unittest.TestCase):
    def _hb(
        self,
        minute: int,
        *,
    ...
    def test_compaction_window_passes_when_incidents_and_latest_present(self) -> None:
        with tempfile.TemporaryDirectory() as td:
            ...
            raw_rows = [
                ...
                self._hb(1, status="failed", strict_fail_count=1, contract_complete=False, clickthrough_ok=False),
                self._hb(2, status="degraded", contract_complete=False, clickthrough_ok=False),
                self._hb(3, status="ok", clickthrough_ok=False),
            ...
            self.assertEqual(int(report["nudge_clickthrough_violations_missing_in_compacted"]), 0)
            self.assertEqual(bool(report["checks"]["latest_present_ok"]), True)
            self.assertEqual(list(report.get("strict_fail_reasons") or []), [])

    def test_compaction_window_strict_fails_when_clickthrough_incident_missing(self) -> None:
        with tempfile.TemporaryDirectory() as td:
            td_path = Path(td)
            raw_path = td_path / "raw.jsonl"
            compacted_path = td_path / "compacted.jsonl"
            ...
            raw_rows = [
                self._hb(0, status="ok"),
                self._hb(1, status="ok", clickthrough_ok=False),
            ...
            self.assertIn("nudge_clickthrough_violations_underreported_in_compacted", reasons)
