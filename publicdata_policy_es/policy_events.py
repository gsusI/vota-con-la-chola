...
_MONEY_DOMAIN_METADATA_BY_KEY: dict[str, tuple[str, str]] = {
    MONEY_DOMAIN_CANONICAL_KEY: (
        "Impuestos, gasto público y sostenibilidad fiscal",
        "Dominio para contratación pública y subvenciones públicas.",
    ),
}


def _money_institution_level(name: str) -> str:
    key = normalize_key_part(name)
    if key.startswith(("ministerio ", "ministerio de", "jefatura del estado", "cortes generales", "tribunal constitucional", "administracion general del estado")):
        return "nacional"
    if key.startswith(("junta ", "generalitat", "xunta ", "gobierno vasco", "gobierno de navarra", "comunidad ", "principado de asturias", "consell ", "diputacion foral")):
        return "autonomico"
    return "nacional"
...
def _resolve_or_seed_domain_id(
    ...
    *,
    ...
    label, description = _MONEY_DOMAIN_METADATA_BY_KEY.get(
        key,
        (key.replace("_", " ").capitalize(), "Dominio inferido automaticamente para eventos de politica de dinero."),
    )
...
def _money_institution_name(source_id: str, payload: dict[str, Any]) -> str | None:
    source_id_norm = normalize_ws(source_id)
    candidates: tuple[str, ...]
    if source_id_norm.startswith("placsp_"):
        candidates = (
            "organo_contratacion",
            "contracting_authority",
            "organoContratacion",
        )
    elif source_id_norm.startswith("bdns_"):
        candidates = (
            "organo_convocante",
            "granting_body",
            "organoConvocante",
...
def _resolve_or_seed_money_institution_id(
    conn: sqlite3.Connection,
    ...
    *,
    ...
    name = normalize_ws(str(institution_name or ""))
    if not name:
        return None, False

    level = _money_institution_level(name)
    ...
    row = conn.execute(
        ...
        """,
        (name, level, now_iso, now_iso),
    ).fetchone()
    if row is None:
        return None, False
    institution_id = int(row["institution_id"])
    cache[cache_key] = institution_id
...
def _boe_department_level(department_name: str) -> str:
    key = normalize_key_part(department_name)
    if key.startswith(("ministerio ", "jefatura del estado", "cortes generales", "tribunal constitucional")):
        return "nacional"
    if key.startswith("comunidad "):
        return "autonomico"
    return "unknown"


def _upsert_boe_department_institution(conn: sqlite3.Connection, department_name: str, now_iso: str) -> int | None:
    name = normalize_ws(department_name)
    if not name:
        return None
    level = _boe_department_level(name)
    row = conn.execute(
        """
        INSERT INTO institutions (
          name, level, admin_level_id, territory_code, territory_id, created_at, updated_at
...
def backfill_moncloa_policy_events(
    ...
    *,
    ...
    stats: dict[str, Any] = {
        ...
        "policy_domains_seeded": 0,
        "policy_institutions_seeded": 0,
        "policy_events_with_institution_id": 0,
...
def backfill_money_policy_events(
    ...
    *,
    ...
    for row in rows:
        ...
        institution_name = _money_institution_name(source_id, payload)
        institution_id, seeded_institution = _resolve_or_seed_money_institution_id(
            conn,
            institution_cache,
            institution_name,
            now_iso=now_iso,
        )
        if seeded_institution:
            stats["policy_institutions_seeded"] += 1
        if institution_id is not None:
            stats["policy_events_with_institution_id"] += 1
        ...
        domain_id, seeded_domain = _resolve_or_seed_domain_id(
            conn,
            domain_cache,
            domain_key,
            now_iso=now_iso,
...
def backfill_boe_policy_events(
    ...
    *,
    ...
    for row in rows:
        ...
        if summary == "":
            summary = None
        department_name = normalize_ws(str(payload.get("department_name") or ""))
        institution_id = _upsert_boe_department_institution(conn, department_name, now_iso) if department_name else None
