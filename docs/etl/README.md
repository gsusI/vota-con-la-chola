...
- Contrato de snapshot citizen (`scripts/export_citizen_snapshot.py`): `meta.quality` publica conteos de stance, `% clear/unknown`, promedio de confianza en celdas con señal y tiers `high/medium/low/none`; validable con `scripts/validate_citizen_snapshot.py` (ahora sobre schema Pydantic en `etl/citizen_snapshot_schema.py`).
- Integraciones opcionales para reducir superficie propia: Datasette (`scripts/run_datasette_explorer.py`, `requirements-explorer.txt`) para browsing genérico de SQLite; Prefect (`scripts/run_source_scrape_queue_prefect.py`, `requirements-ops.txt`) para orchestration del scrape queue; Browserbase (`scripts/create_browserbase_session.py`) y Apify (`scripts/run_apify_actor.py`) para sesiones anti-bot/actors gestionados; Label Studio (`scripts/export_initdoc_extraction_label_studio_tasks.py`, `scripts/apply_initdoc_extraction_label_studio_reviews.py`) para revisión humana con menos CSV glue.
...
- Si prefieres Label Studio en vez de round-trip CSV, usa:
  - `python3 scripts/export_initdoc_extraction_label_studio_tasks.py --db <db> --source-id parl_initiative_docs --only-needs-review --out docs/etl/sprints/<SPRINT>/exports/initdoc_extraction_review_queue.label_studio.json --config-out docs/etl/sprints/<SPRINT>/exports/initdoc_extraction_review_label_studio_config.xml`
  - importa ese JSON en Label Studio y exporta el resultado anotado
  - reaplica con `python3 scripts/apply_initdoc_extraction_label_studio_reviews.py --db <db> --source-id parl_initiative_docs --in <label_studio_export.json> --out docs/etl/sprints/<SPRINT>/evidence/initdoc_extraction_review_apply.json`
  - atajos: `just parl-export-initdoc-extraction-label-studio`, `just parl-apply-initdoc-extraction-label-studio`, `just parl-apply-initdoc-extraction-label-studio-dry-run`
