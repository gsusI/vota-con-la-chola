...
- Fresh official BDNS now closes the real million-row acquisition-to-analytics capacity slice without identity suppression: `1,419/1,419` date-partitioned pages, `1,360,382` distinct source rows/URLs/version sightings, `1,080,788,680` raw bytes, zero retries/dead work, `89/89` selected daily windows complete, SQLite `quick_check=ok`, FK `0`; v6 writes `14` Parquet files totaling `42,955,289` bytes and independently validates all `1,360,382` beneficiary names plus all `163,270` source-published identifiers exactly. Unchanged replay hardlinks `14/14` files. Registry/readiness validate `6/6` real corpora; `3` exceed one million and `0` are promoted because representation/history, origin, restore, and revision gates remain open.
