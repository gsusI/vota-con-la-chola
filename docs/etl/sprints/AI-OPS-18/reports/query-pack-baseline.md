# AI-OPS-18 Query Pack (Baseline + Gates)
...
" > docs/etl/sprints/AI-OPS-18/exports/programas_metrics.csv
```
...
python3 scripts/export_explorer_sources_snapshot.py \
  ...
  --out docs/etl/sprints/AI-OPS-18/evidence/status-postrun.json

python3 scripts/export_explorer_sources_snapshot.py \
  --db etl/data/staging/politicos-es.db \
  --out docs/gh-pages/explorer-sources/data/status.json

python3 - <<'PY' > docs/etl/sprints/AI-OPS-18/evidence/status-parity-postrun.txt
import json
from pathlib import Path

final = json.loads(Path('docs/etl/sprints/AI-OPS-18/evidence/status-postrun.json').read_text(encoding='utf-8'))
...
overall_match = True
print('# AI-OPS-18 status parity summary')
