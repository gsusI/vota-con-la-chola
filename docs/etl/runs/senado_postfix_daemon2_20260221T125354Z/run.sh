...
PY
  ...
  AFTER=$(q_done)
  echo "ts=$(date -u +%Y-%m-%dT%H:%M:%SZ) round=$r urls_to_fetch=$URLS fetched_ok=$FETCH_OK delta=$((AFTER-BEFORE)) done=$AFTER/$TOTAL"
  if [ $((AFTER-BEFORE)) -gt 0 ]; then sleep 120; else sleep 360; fi
done
