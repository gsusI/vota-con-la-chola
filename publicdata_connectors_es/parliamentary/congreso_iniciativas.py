...
from publicdata_core.util import now_utc_iso, sha256_bytes, stable_json
...
# The catalog page contains multiple export links like:
# /webpublica/opendata/iniciativas/ProyectosDeLey__20260212050027.json
INICIATIVAS_JSON_RE = re.compile(
    r'(?P<href>/webpublica/opendata/iniciativas/[^"\' ]+__\d{14}\.json)\b'
)


class CongresoIniciativasConnector(BaseConnector):
    ...
    def extract(
        ...
        if from_file:
            ...
            meta = {
                "source": "congreso_iniciativas_from_file",
        ...
        hrefs = [m.group("href") for m in INICIATIVAS_JSON_RE.finditer(html)]
        json_urls: list[str] = []
        seen: set[str] = set()
        for href in hrefs:
        ...
        for url in json_urls:
            try:
                ...
                category = Path(url).name.split("__", 1)[0]
                downloaded.append({"url": url, "category": category, "items": len(items)})
                for item in items:
                    if not isinstance(item, dict):
                        continue
                    records.append({"list_url": url, "category": category, "payload": item})
            except Exception as exc:  # noqa: BLE001
        ...
        meta = {
            "source": "congreso_iniciativas_catalog",
            "list_url": resolved_url,
            "json_urls_total": len(json_urls),
