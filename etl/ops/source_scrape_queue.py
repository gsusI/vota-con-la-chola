...
def _render_command(command: str, *, db_path: Path, snapshot_date: str) -> str:
...
def _normalize_command(command: str, *, db_path: Path, snapshot_date: str) -> list[str]:
    rendered = _render_command(command, db_path=db_path, snapshot_date=snapshot_date)
...
def _prerequisite_source_ids(item: dict[str, Any]) -> list[str]:
...
def _pre_commands(item: dict[str, Any]) -> list[str]:
    execution = item.get("execution") if isinstance(item.get("execution"), dict) else {}
...
def _sort_items_by_dependencies(items: list[dict[str, Any]]) -> list[dict[str, Any]]:
    ...
    def visit(item: dict[str, Any]) -> None:
        ...
        for dep_source_id in _prerequisite_source_ids(item):
